...
unsafe fn alloc_prof_prev_alloc(
    len: size_t,
    #[cfg(php_debug)] filename: *const c_char,
    #[cfg(php_debug)] lineno: u32,
    #[cfg(php_debug)] orig_filename: *const c_char,
    ...
    let alloc = |zend_mm_state: ZendMMState| {
        ...
        #[cfg(php_debug)]
        let ptr = zend_mm_state.prev_custom_mm_alloc.unwrap_unchecked()(
            len,
            filename,
            lineno,
            orig_filename,
...
unsafe fn alloc_prof_orig_alloc(
    len: size_t,
    #[cfg(php_debug)] filename: *const c_char,
    #[cfg(php_debug)] lineno: u32,
    #[cfg(php_debug)] orig_filename: *const c_char,
    ...
    #[cfg(php_debug)]
    let ptr: *mut c_void = zend::_zend_mm_alloc(
        tls_zend_mm_state_get!(prev_heap),
        len,
        filename,
        lineno,
        orig_filename,
...
unsafe extern "C" fn alloc_prof_free(
    ...
    #[cfg(php_debug)]
    tls_zend_mm_state_get!(free)(ptr, filename, lineno, orig_filename, orig_lineno);
}

unsafe fn alloc_prof_prev_free(
    ptr: *mut c_void,
    #[cfg(php_debug)] filename: *const c_char,
    #[cfg(php_debug)] lineno: u32,
    #[cfg(php_debug)] orig_filename: *const c_char,
    ...
    let free = |zend_mm_state: ZendMMState| {
        ...
        #[cfg(php_debug)]
        (zend_mm_state.prev_custom_mm_free.unwrap_unchecked())(
            ptr,
            filename,
            lineno,
            orig_filename,
...
unsafe fn alloc_prof_orig_free(
    ptr: *mut c_void,
    #[cfg(php_debug)] filename: *const c_char,
    #[cfg(php_debug)] lineno: u32,
    #[cfg(php_debug)] orig_filename: *const c_char,
...
unsafe extern "C" fn alloc_prof_realloc(
    ...
    #[cfg(php_debug)]
    let ptr = tls_zend_mm_state_get!(realloc)(
        prev_ptr,
        len,
        filename,
        lineno,
...
unsafe fn alloc_prof_prev_realloc(
    prev_ptr: *mut c_void,
    len: size_t,
    #[cfg(php_debug)] filename: *const c_char,
    #[cfg(php_debug)] lineno: u32,
    ...
    let realloc = |zend_mm_state: ZendMMState| {
        ...
        #[cfg(php_debug)]
        let ptr = zend_mm_state.prev_custom_mm_realloc.unwrap_unchecked()(
            prev_ptr,
            len,
            filename,
            lineno,
...
unsafe fn alloc_prof_orig_realloc(
    prev_ptr: *mut c_void,
    len: size_t,
    #[cfg(php_debug)] filename: *const c_char,
    #[cfg(php_debug)] lineno: u32,
    ...
    #[cfg(not(php_debug))]
    let result = zend::_zend_mm_realloc(tls_zend_mm_state_get!(prev_heap), prev_ptr, len);
    #[cfg(php_debug)]
    let result = zend::_zend_mm_realloc(
        tls_zend_mm_state_get!(prev_heap),
        prev_ptr,
        len,
        filename,
