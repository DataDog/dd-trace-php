...
fn build_frame(
...
) -> Result<Option<ZendFrame>, ProfileError> {
    ...
    let name = match name_cow {
        Some(ref s) if !s.is_empty() => strings.try_insert(s)?,
        _ => StringId::EMPTY,
    };
    ...
    if name != StringId::EMPTY || file_name != StringId::EMPTY {
        let function = Function {
            name,
            system_name: StringId::EMPTY,
            file_name,
        };
        let set = dict.functions();
        let function_id = Some(set.try_insert(function)?.into_raw());
...
mod detail {
    ...
    fn handle_function_cache_slot(
        func: &zend_function,
        execute_data: &zend_execute_data,
        cache: &mut Option<FunctionId>,
        dict: &ProfilesDictionary,
    ) -> Result<Option<ZendFrame>, ProfileError> {
        if let Some(function_id) = cache {
