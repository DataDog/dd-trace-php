...
impl KeyMapCache {
    ...
    fn read_tracer_process_context(&self) -> std::io::Result<Vec<String>> {
        ...
        let mapping = unsafe {
            libdd_library_config::otel_process_ctx::ProcessContextMapping::from_raw_parts(
                globals.otel_process_context_base.cast_mut(),
                globals.otel_process_context_len,
            )
        }?;
        libdd_library_config::otel_process_ctx::read(mapping).map(|(context, _)| {
...
impl Profiler {
    ...
    fn common_context(n_extra_labels: usize) -> SampleContext {
        ...
        let otel_context = current_otel_sample_context();
        let thread_id = otel_context
            .as_ref()
