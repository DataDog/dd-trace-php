...
system_time_result_t datadog_php_system_time_now(struct timespec *now) {
  return timespec_get(now, TIME_UTC) == TIME_UTC ? DATADOG_PHP_SYSTEM_TIME_OK
 ...
 */
system_time_result_t datadog_php_system_time_now(struct timespec *now) {
  return clock_gettime(CLOCK_REALTIME, now) == 0 ? DATADOG_PHP_SYSTEM_TIME_OK
                                                 : DATADOG_PHP_SYSTEM_TIME_ERR;
}
#else
...
_Static_assert(DATADOG_HAVE_CLOCK_GETTIME,
               "HAVE_PTHREAD_GETCPUCLOCKID assumes DATADOG_HAVE_CLOCK_GETTIME");
