...
logger_t datadog_php_logger_ctor(int descriptor, log_level_t log_level,
...
int64_t datadog_php_logv(datadog_php_logger *logger,
  ...
  if (!datadog_php_logger_valid(logger))
    return -1;
  if (logger->mutex) {
    if (pthread_mutex_lock(logger->mutex) == 0) {
...
log_level_t datadog_php_log_level_detect(string_view_t val) {
  ...
  for (i = 0; i != n_options; ++i) {
    if (datadog_php_string_view_eq(options[i].str, level_string)) {
      break;
    }
  }
