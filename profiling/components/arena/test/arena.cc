...
TEST_CASE("generic alignment", "[arena]") {
  CHECK(datadog_php_arena_align_diff(reinterpret_cast<uintptr_t>(buffer + 0),
                                     4) == 0u);
  CHECK(datadog_php_arena_align_diff(reinterpret_cast<uintptr_t>(buffer + 1),
                                     4) == 3u);
  CHECK(datadog_php_arena_align_diff(reinterpret_cast<uintptr_t>(buffer + 2),
                                     4) == 2u);
  CHECK(datadog_php_arena_align_diff(reinterpret_cast<uintptr_t>(buffer + 3),
                                     4) == 1u);
  CHECK(datadog_php_arena_align_diff(reinterpret_cast<uintptr_t>(buffer + 4),
                                     4) == 0u);
  CHECK(datadog_php_arena_align_diff(reinterpret_cast<uintptr_t>(buffer + 5),
                                     4) == 3u);
}
