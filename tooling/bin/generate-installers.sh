...
if echo "${release_version}" | grep -qE '\+' && [ -n ${CI_JOB_ID:-} ]; then
  ...
  replacement="define('RELEASE_URL_PREFIX', getenv('DD_TEST_INSTALLER_REPO')"
  replacement="${replacement} ? getenv('DD_TEST_INSTALLER_REPO') . '/releases/download/' . RELEASE_VERSION . '/'"
  replacement="${replacement} : 'https://s3.us-east-1.amazonaws.com/dd-trace-php-builds/' . RELEASE_VERSION . '/')"
