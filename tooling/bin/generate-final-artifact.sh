...
for architecture in "${architectures[@]}"; do
    ...
    if [ "$architecture" = "x86_64" ]; then
        ...
        gnu_suffixes=("" "-zts" "-debug")
        # musl_suffixes=("-alpine")
        musl_suffixes=()
    ...
    for php_api in "${php_apis[@]}"; do
        ...
        # Gnu
        for suffix in "${gnu_suffixes[@]}"; do
            cp ./extensions_${architecture}/ddtrace-$php_api$suffix.so ${tmp_folder_final_gnu_trace}/ext/$php_api/ddtrace$suffix.so;
        done;

        # Musl
        for suffix in "${musl_suffixes[@]}"; do
            cp ./extensions_${architecture}/ddtrace-$php_api$suffix.so ${tmp_folder_final_musl_trace}/ext/$php_api/ddtrace$suffix.so;
        done;
    done
