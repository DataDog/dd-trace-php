...
|--------|-------|-------------|
| `test appsec extension: [{ver}, {arch}, debug]` | `datadog/dd-trace-ci:php-{ver}_bookworm-9` | Builds appsec PHP extension + runs phpunit `.phpt` tests |
...
| `test appsec helper asan` | `datadog/dd-trace-ci:bookworm-9` | Builds C++ helper with ASAN, runs gtest suite |
| `appsec lint` | `datadog/dd-trace-ci:php-8.3_bookworm-9` | clang-format + clang-tidy |
| `appsec code coverage` | `datadog/dd-trace-ci:php-8.3_bookworm-9` | Coverage instrumented build (not needed locally) |
...
.claude/ci/dockerh --cache appsec-ext-8.3-debug-arm64 --overlayfs --php debug \
  datadog/dd-trace-ci:php-8.3_bookworm-9 --platform linux/arm64 -- bash -c '...'
```
...
The C++ helper tests use the `bookworm-9` image (no PHP needed). The
