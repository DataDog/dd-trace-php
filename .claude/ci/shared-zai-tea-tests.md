...
|--------|-------|-------------|
| `Build & Test Tea` | `dd-trace-ci:php-{ver}_bookworm-11` | Builds the TEA (Test Execution Abstraction) library from `tea/`, runs its ctest suite, installs artifacts for downstream jobs |
| `Zend Abstract Interface Tests: [{ver}, {variant}]` | `dd-trace-ci:php-{ver}_bookworm-11` | Builds and tests the ZAI library (`zend_abstract_interface/`) against a specific PHP variant |
| `Extension Tea Tests: [{ver}, {variant}]` | `dd-trace-ci:php-{ver}_bookworm-11` | Builds ddtrace.so via `make install`, then builds and runs the extension-level TEA tests in `tests/tea/` |
...
| `C components ASAN` | `dd-trace-ci:centos-7`, `dd-trace-ci:php-compile-extension-alpine`, `dd-trace-ci:bookworm-11` | Builds C components (`components/`) with ASAN (on Debian) or plain Debug (on CentOS/Alpine), runs ctest |
| `C components UBSAN` | `dd-trace-ci:bookworm-11` | Builds C components with UBSAN, runs ctest with `--repeat until-fail:10` |
| `Configuration Consistency` | `dd-trace-ci:php-{latest}_bookworm-11` | Runs `tooling/generate-supported-configurations.sh` and verifies `metadata/supported-configurations.json` is up-to-date |
...
- **C components ASAN**: three images (centos-7, alpine, bookworm-11);
  ASAN toolchain only on Debian (bookworm).
- **C components UBSAN**: bookworm-11 only.
...
.claude/ci/dockerh --cache components-asan \
  datadog/dd-trace-ci:bookworm-11 -- bash -c '
set -e
...
.claude/ci/dockerh --cache config-consistency --overlayfs \
  --php nts \
  datadog/dd-trace-ci:php-8.5_bookworm-11 -- bash -c '
...
- **C components tests do not require PHP.** The `bookworm-11` base
