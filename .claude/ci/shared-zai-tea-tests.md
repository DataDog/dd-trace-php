...
```bash
.claude/ci/dockerh --cache tea-8.3-debug --php debug \
...
```bash
.claude/ci/dockerh --cache ext-tea-8.3-debug --writable-tree --php debug \
  datadog/dd-trace-ci:php-8.3_bookworm-6 -- bash -c '
set -e
make install
...
`--writable-tree` is needed because `make install` writes to the PHP
extension directory and generates build artifacts in the source tree.
...
```bash
.claude/ci/dockerh --cache config-consistency --no-cache-overlay \
  --writable-tree --php nts \
  datadog/dd-trace-ci:php-8.5_bookworm-6 -- bash -c '
bash tooling/generate-supported-configurations.sh
'
```

`--writable-tree` is required because the script writes `ext/version.h` as a side effect.
