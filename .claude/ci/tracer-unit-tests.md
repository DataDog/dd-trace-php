...
|--------|-------|-------------|
| `compile extension: debug` | `dd-trace-ci:php-{ver}_bookworm-9` | Compiles ddtrace.so in debug mode; produces artifact consumed by all test jobs below |
| `compile extension: debug-zts-asan` | same | Compiles ddtrace.so with ASAN+ZTS; used by ASAN test jobs |
| `Unit tests: [{ver}]` | `dd-trace-ci:php-{ver}_bookworm-9` | Runs PHPUnit `--testsuite=unit` |
