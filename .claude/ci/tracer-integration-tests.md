...
|--------|-------|-------------|
| `test_integrations_amqp2: [{php}]` | `datadog/dd-trace-ci:php-{php}_bookworm-9` | AMQP v2 (RabbitMQ) integration tests |
...
- **phpredis version-specific .so files**: `test_integrations_phpredis3`
  ...
  `TEST_EXTRA_INI=-d extension=redis-X.Y.Z.so`. The `bookworm-9` CI image
