...
static int zai_hook_resolve_impl(zval *zv ZAI_TSRMLS_DC) {
    ...
    if (hook->scope) {
        zend_class_entry *scope =
            zai_symbol_lookup_class(ZAI_SYMBOL_SCOPE_GLOBAL, NULL, zai_hook_string_cast(hook->scope) ZAI_TSRMLS_CC);
        if (!scope) {
            /* class not available */
            return ZEND_HASH_APPLY_KEEP;
        }

        function = zai_symbol_lookup_function(ZAI_SYMBOL_SCOPE_CLASS, scope,
                                              zai_hook_string_cast(hook->function) ZAI_TSRMLS_CC);
    } else {
        function = zai_symbol_lookup_function(ZAI_SYMBOL_SCOPE_GLOBAL, NULL,
                                              zai_hook_string_cast(hook->function) ZAI_TSRMLS_CC);
    }
    ...
    if (!zai_hook_table_find(&zai_hook_resolved, address, &table)) {
        ...
        if (!zai_hook_table_insert_at(&zai_hook_resolved, address, &resolving, sizeof(HashTable), (void **)&table)) {
    ...
    ZAI_HOOK_FOREACH(hooks, hook, {
        switch (hook->type) {
            ...
            case ZAI_HOOK_USER: {
                ...
                zval *rvu;
                ZAI_VALUE_INIT(rvu);
                zai_symbol_call(This ? ZAI_SYMBOL_SCOPE_OBJECT : ZAI_SYMBOL_SCOPE_GLOBAL, This ? This : NULL,
                                ZAI_SYMBOL_FUNCTION_CLOSURE, &hook->begin.u, &rvu ZAI_TSRMLS_CC, 0);
    ...
    ZAI_HOOK_FOREACH(hooks, hook, {
        switch (hook->type) {
            ...
            case ZAI_HOOK_USER: {
                ...
                ZAI_VALUE_INIT(rvu);
                zai_symbol_call(This ? ZAI_SYMBOL_SCOPE_OBJECT : ZAI_SYMBOL_SCOPE_GLOBAL, This ? This : NULL,
                                ZAI_SYMBOL_FUNCTION_CLOSURE, &hook->end.u, &rvu ZAI_TSRMLS_CC, rv != NULL ? 1 : 0,
                                rv != NULL ? &rv : NULL);
                ZAI_VALUE_DTOR(rvu);
            } break;
        }
...
bool zai_hook_install(zai_hook_type_t type, zai_string_view scope, zai_string_view function, zai_hook_begin begin,
                      zai_hook_end end, void *fixed, size_t dynamic ZAI_TSRMLS_DC) {
