...
static inline zai_str zai_str_new(const char *ptr, size_t len) {
    return ptr ? (zai_str)ZAI_STR_FROM_RAW_PARTS(ptr, len) : (zai_str)ZAI_STR_EMPTY;
}
...
static inline zai_str zai_str_from_cstr(const char *cstr) {
    return cstr ? (zai_str)ZAI_STR_FROM_CSTR(cstr) : (zai_str)ZAI_STR_EMPTY;
}
...
inline zai_str zai_str_from_zstr(zend_string *zstr) {
    return zstr ? (zai_str)ZAI_STR_FROM_ZSTR(zstr) : (zai_str)ZAI_STR_EMPTY;
}
