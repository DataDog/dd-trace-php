...
pub struct Storage {
    ...
    header: ThinHeader,
    /// The bytes of the strings are stored here. Immutable after creation.
    data: str,
}
...
/// # Safety
/// The strings are immutable and can therefore be sent between threads.
unsafe impl Send for ThinHeader {}

/// # Safety
/// The strings are immutable and can therefore be shared amongst threads.
unsafe impl Sync for ThinHeader {}

/// ConstStorage representing the empty string "".
pub static EMPTY: ConstStorage<0> = ConstStorage::from_str("");

impl Storage {
    /// Create a Storage reference from a non-null pointer.
    /// # Safety
...
impl<const N: usize> ConstStorage<N> {
    /// Create a [ConstStorage] from the provided string.
    /// # Panic
    /// Panics if the length of the string does not match the provided N.
    pub const fn from_str(str: &str) -> ConstStorage<N> {
        ...
        // SAFETY: the bytes of the str are initialized, the length of the
        // string is guarded to match the length of the array, the str is
        // valid for reads, and strings are always properly aligned since
        // their alignment is 1.
        let data = unsafe { ptr::read(str.as_ptr().cast::<[u8; N]>()) };
        ConstStorage::<N> { header, data }
    }

    /// Get a [Storage] reference to the same data held by this [ConstStorage].
    ...
    /// Get a `&str` to the data held by this [ConstStorage].
    /// This method exists because From/Into are not const.
