...
pub struct ThinStr<'a> {
    ...
    /// because of the DST), so we store a pointer to the header instead.
...
impl ThinStr<'_> {
    /// Creates a [ThinStr] that represents the empty string "".
    pub fn new() -> Self {
        Self::from_const_storage(&EMPTY)
    }

    /// Creates a [ThinStr] from the provided pointer, but it must be a
    /// pointer to a valid [Storage] object. Since [Storage] objects are
    /// immutable, we can always make a new immutable references to the same
    /// storage object without running afoul of aliasing rules. However,
    /// care does need to be taken to make sure the lifetime is correctly
    /// bound.
    /// # Safety
    /// The thin pointer should point to a valid [Storage] object. The lifetime
    /// of the created [ThinStr] must not outlive the data it refers to.
    pub const unsafe fn from_header(ptr: ptr::NonNull<ThinHeader>) -> Self {
        let _marker = marker::PhantomData;
        Self { ptr, _marker }
    }

    /// Creates a [ThinStr] from a &[ConstStorage].
    /// This method exists because From/Into are not const.
...
/// # Safety
/// Static strings are immutable and can therefore be sent between threads.
unsafe impl Send for ThinStr<'static> {}

/// # Safety
/// Static strings are immutable and can therefore be shared amongst threads.
