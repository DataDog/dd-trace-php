...
static struct curl_slist *dd_agent_headers_alloc(void) {
    ...
    if (get_global_DD_TRACE_STATS_COMPUTATION_ENABLED() || !get_global_DD_APM_TRACING_ENABLED()) {
        dd_append_header(&list, "Datadog-Client-Computed-Stats", ZEND_STRL("true"));
    }
    if (get_global_DD_TRACE_STATS_COMPUTATION_ENABLED()) {
        dd_append_header(&list, "Datadog-Client-Computed-Top-Level", ZEND_STRL("yes"));
    }
