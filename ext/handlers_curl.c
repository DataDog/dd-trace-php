...
static void dd_ch_store_headers(zend_object *ch, HashTable *headers) {
    ...
    if (!zend_weakrefs_hash_add_ptr(&DDTRACE_G(curl_headers), ch, new_headers)) {
        zend_hash_index_update_ptr(&DDTRACE_G(curl_headers), zend_object_to_weakref_key(ch), new_headers);
    }
}

static void dd_ch_delete_headers(zend_object *ch) { zend_weakrefs_hash_del(&DDTRACE_G(curl_headers), ch); }

static void dd_ch_duplicate_headers(zend_object *ch_orig, zend_object *ch_new) {
    HashTable *headers = zend_hash_index_find_ptr(&DDTRACE_G(curl_headers), zend_object_to_weakref_key(ch_orig));
    if (headers) {
...
static void dd_inject_distributed_tracing_headers(zend_object *ch) {
    ...
    if ((dd_header_array = zend_hash_index_find_ptr(&DDTRACE_G(curl_headers), zend_object_to_weakref_key(ch)))) {
...
static void dd_multi_add_handle(zend_object *mh, zend_object *ch) {
    HashTable *handles = zend_hash_index_find_ptr(&DDTRACE_G(curl_multi_handles), zend_object_to_weakref_key(mh));
    if (!handles) {
        ALLOC_HASHTABLE(handles);
        zend_hash_init(handles, 8, NULL, ZVAL_PTR_DTOR, 0);
        zend_weakrefs_hash_add_ptr(&DDTRACE_G(curl_multi_handles), mh, handles);
    }
    ...
    if (handles && zend_hash_num_elements(handles) > 0) {
        ...
        ZEND_HASH_FOREACH_PTR(handles, ch) {
            if (DDTRACE_G(curl_multi_injecting_spans) && Z_TYPE(DDTRACE_G(curl_multi_injecting_spans)->val) == IS_ARRAY) {
                ddtrace_span_data *span = ddtrace_open_span(DDTRACE_INTERNAL_SPAN, false);
        ...
        zend_weakrefs_hash_del(&DDTRACE_G(curl_multi_handles), mh);
    }
}
...
static HashTable *ddtrace_curl_multi_get_gc(zend_object *object, zval **table, int *n) {
    ...
    HashTable *handles = zend_hash_index_find_ptr(&DDTRACE_G(curl_multi_handles), zend_object_to_weakref_key(object));
    if (handles) {
...
ZEND_FUNCTION(ddtrace_curl_multi_close) {
    ...
    if (dd_load_curl_integration() &&
        ...
        zend_weakrefs_hash_del(&DDTRACE_G(curl_multi_handles), Z_OBJ_P(z_mh));
    }
...
void ddtrace_curl_handlers_rshutdown(void) {
    dd_curl_destroy_weakref_ht(&DDTRACE_G(curl_headers));
    dd_curl_destroy_weakref_ht(&DDTRACE_G(curl_multi_handles));
}
