...
static void dd_decide_on_sampling(ddtrace_span_fci *span TSRMLS_DC) {
    ...
    if (priority == DDTRACE_UNKNOWN_PRIORITY_SAMPLING) {
        ...
        for (zend_hash_internal_pointer_reset_ex(rules, &pos);
            ...
            if (zend_hash_find(Z_ARRVAL_PP(rule), "service", sizeof("service"), (void **)&rule_pattern) == SUCCESS) {
                rule_matches &= dd_rule_matches(*rule_pattern, ddtrace_spandata_property_service(&span->span) TSRMLS_CC);
            }
            ...
            zval **sample_rate_zv;
            if (rule_matches && zend_hash_find(Z_ARRVAL_PP(rule), "sample_rate", sizeof("sample_rate"), (void **)&sample_rate_zv) == SUCCESS) {
        ...
        add_assoc_double_ex(ddtrace_spandata_property_metrics(&span->span), "_dd.rule_psr", sizeof("_dd.rule_psr"), sample_rate);
    ...
    add_assoc_long_ex(ddtrace_spandata_property_metrics(&span->span), "_sampling_priority_v1", sizeof("_sampling_priority_v1"), priority);
}
...
long ddtrace_fetch_prioritySampling_from_root(TSRMLS_D) {
    ...
    if (zend_hash_find(root_metrics, "_sampling_priority_v1", sizeof("_sampling_priority_v1"), (void **)&priority_zv) == FAILURE) {
...
void ddtrace_set_prioritySampling_on_root(long priority TSRMLS_DC) {
    ...
    if (priority == DDTRACE_UNKNOWN_PRIORITY_SAMPLING) {
        ...
        zend_hash_update(root_metrics, "_sampling_priority_v1", sizeof("_sampling_priority_v1"), &zv, sizeof(zval *), NULL);
    }
}
