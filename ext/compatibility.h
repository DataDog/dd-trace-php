...
#if PHP_VERSION_ID >= 70100 && PHP_VERSION_ID < 70200 && defined(ZTS)
/* PHP 7.1 ZTS destroys its process-wide interned strings before destroying the
 * main thread's copied class table. A typed-property stub may pass ZSTR_KNOWN()
 * here, leaving the copied property info pointing at an already-freed string
 * during tsrm_shutdown(). Store a regular persistent string instead. */
