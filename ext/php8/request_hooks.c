...
void dd_request_init_hook_rinit(TSRMLS_D) {
    ...
    {
        const char *hook = DDTRACE_G(request_init_hook);
        zend_string *hook_zstr = zend_string_init(hook, strlen(hook), 1);
        php_stat(hook_zstr, FS_EXISTS, &exists_flag);
        zend_string_release_ex(hook_zstr, 1);
    }
#else
