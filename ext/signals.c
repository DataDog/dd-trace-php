...
static void dd_crasht_do_init(ddog_crasht_EndpointConfig endpoint_config, void *userdata) {
    ...
    uint32_t sidecar_master_pid = datadog_sidecar_active_mode == DD_SIDECAR_CONNECTION_THREAD
        ? (uint32_t)datadog_sidecar_master_pid
        : 0;
    dd_crashtracker_check_result(
            datadog_crasht_init_without_receiver(args->config, args->metadata, sidecar_master_pid),
