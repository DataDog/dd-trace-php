...
void ddtrace_serialize_span_to_array(ddtrace_span_data *span, zval *array) {
    ...
    if (operation_name && Z_TYPE_P(operation_name) == IS_STRING) {
        ...
        prop_name = zend_hash_str_update(Z_ARR_P(el), ZEND_STRL("name"), operation_name);
        zend_hash_str_del(meta, ZEND_STRL("operation.name"));
    } else {
        prop_name = &span->property_name;
    ...
    if (resource_name && Z_TYPE_P(resource_name) == IS_STRING) {
        ZVAL_COPY(&prop_resource_as_string, resource_name);
    ...
    if (span_type && Z_TYPE_P(span_type) == IS_STRING) {
        ZVAL_COPY(&prop_type_as_string, span_type);
    ...
    zend_array *metrics = ddtrace_property_array(&span->property_metrics);
    zval *_analytics_event_metrics = zend_hash_str_find(metrics, ZEND_STRL("analytics.event"));
    zval *analytics_event = _analytics_event_metrics ? _analytics_event_metrics : zend_hash_str_find(meta, ZEND_STRL("analytics.event"));
    if (analytics_event) {
        zval sample_rate;
        if (Z_TYPE_P(analytics_event) == IS_STRING) {
            ZVAL_DOUBLE(&sample_rate, zend_string_equals_literal_ci(Z_STR_P(analytics_event), "true") ? 1.0 : 0.0);
        } else {
            ZVAL_DOUBLE(&sample_rate, zval_get_double(analytics_event));
        }
        zend_hash_str_update(metrics, ZEND_STRL("_dd1.sr.eausr"), &sample_rate);
    }
    zend_hash_str_del(meta, ZEND_STRL("analytics.event"));
    zend_hash_str_del(metrics, ZEND_STRL("analytics.event"));
