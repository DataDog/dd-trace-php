...
static bool dd_init_live_debugger_probe(const ddog_Probe *probe, dd_probe_def *def) {
    def->probe = *probe;
    ...
    if (target->type_name.tag == DDOG_OPTION_CHAR_SLICE_SOME_CHAR_SLICE) {
        if (!ddog_type_can_be_instrumented(DDTRACE_G(remote_config_state),  target->type_name.some)) {
            return false;
        }
...
static int64_t dd_set_span_probe(const ddog_Probe *probe) {
    ...
    zend_long id;
    if (!dd_init_live_debugger_probe(probe, def)
...
static void dd_submit_probe_eval_error_snapshot(const ddog_Probe *probe, ddog_Vec_SnapshotEvaluationError *error) {
    ...
    ddog_DebuggerPayload *snapshot = ddog_evaluation_error_snapshot(probe,
                                                                              ...
                                                                              error,
                                                                              zend_hrtime());
    ddog_sidecar_send_debugger_datum(&ddtrace_sidecar, ddtrace_sidecar_instance_id, DDTRACE_G(telemetry_queue_id), snapshot);
...
static void dd_log_probe_ensure_payload(dd_log_probe_dyn *dyn, dd_log_probe_def *def, ddog_CharSlice *msg) {
    if (dyn->payload) {
        ...
        dyn->payload = ddog_create_log_probe_snapshot(&def->parent.probe, msg, dd_zend_string_to_CharSlice(dyn->service), DDOG_CHARSLICE_C("php"), zend_hrtime());
    }
}
...
static void dd_log_probe_end(zend_ulong invocation, zend_execute_data *execute_data, zval *retval, void *auxiliary, void *dynamic) {
    ...
    ddog_sidecar_send_debugger_datum(&ddtrace_sidecar, ddtrace_sidecar_instance_id, DDTRACE_G(telemetry_queue_id), dyn->payload);
...
static int64_t dd_set_log_probe(const ddog_Probe *probe, const ddog_MaybeShmLimiter *limiter) {
    ...
    zend_long id;
    if (!dd_init_live_debugger_probe(probe, &def->parent)
        || 0 > (id = zai_hook_install(
            def->parent.scope ? (zai_str) ZAI_STR_FROM_ZSTR(def->parent.scope) : (zai_str) ZAI_STR_EMPTY,
            def->parent.function ? (zai_str) ZAI_STR_FROM_ZSTR(def->parent.function) : (zai_str) ZAI_STR_EMPTY,
            dd_log_probe_begin,
            dd_log_probe_end,
            ZAI_HOOK_AUX(def, dd_span_probe_dtor),
            sizeof(dd_log_probe_dyn)))) {
        dd_span_probe_dtor(def);
        return -1;
    }
