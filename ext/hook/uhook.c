...
void dd_uhook_callback_apply_scope(dd_uhook_callback *cb, zend_class_entry *scope) {
    ...
    void **rt_cache = (void **)&func->op_array.run_time_cache;
#else
    void **rt_cache = (void **)&ZEND_MAP_PTR(func->op_array.run_time_cache);
#endif
    if (GC_REFCOUNT(cb->closure) == 1) {
        ...
        if (!(func->common.fn_flags & ZEND_ACC_HEAP_RT_CACHE)) {
            ...
            *rt_cache = emalloc(func->op_array.cache_size);
        }
    } else {
        zval zv;
    ...
    memset(*rt_cache, 0, func->op_array.cache_size);
end:
