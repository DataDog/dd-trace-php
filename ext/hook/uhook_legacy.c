...
static void dd_uhook(INTERNAL_FUNCTION_PARAMETERS, bool tracing, bool method) {
    ...
    zai_str class_str = method ? (zai_str)ZAI_STR_FROM_ZSTR(class_name) : (zai_str)ZAI_STR_EMPTY;
...
PHP_FUNCTION(dd_untrace) {
    ...
    zai_str class_str = class_name ? (zai_str)ZAI_STR_FROM_ZSTR(class_name) : (zai_str)ZAI_STR_EMPTY;
    zai_str func_str = ZAI_STR_FROM_ZSTR(method_name);
