...
static zend_string *ddtrace_ffe_append_otlp_metrics_path(const char *base_endpoint, size_t base_len) {
...
static zend_string *ddtrace_ffe_build_otlp_metrics_endpoint(const char *scheme, size_t scheme_len, const char *host, size_t host_len) {
...
static zend_string *ddtrace_ffe_otlp_metrics_endpoint_from_agent_config(void) {
    ...
    if (ZSTR_LEN(agent_url) > 0) {
        ...
        if (parsed) {
            ...
            if (parsed->host) {
                zend_string *endpoint = ddtrace_ffe_build_otlp_metrics_endpoint(
...
static zend_string *ddtrace_ffe_otlp_metrics_endpoint(void) {
    ...
    if (base_endpoint && base_endpoint[0] != '\0') {
        return ddtrace_ffe_append_otlp_metrics_path(base_endpoint, strlen(base_endpoint));
    }

    return ddtrace_ffe_otlp_metrics_endpoint_from_agent_config();
}
