...
_CRATE_HUB = "@ddtrace_tracer_crates//:"
_PRODUCT_DATA = PRODUCT_GRAPHS["tracer"]
_SPEC_BY_PACKAGE = {crate.package: crate for crate in LIBDATADOG_CRATES}
_SPECIAL_PATH_TARGETS = {
    "appsec/helper-rust": ":rust_appsec_helper",
    "appsec/third_party/libddwaf-rust/crates/libddwaf": ":rust_native_libddwaf",
    "appsec/third_party/libddwaf-rust/crates/libddwaf-sys": ":rust_native_libddwaf_sys",
    "sidecar": ":rust_ddtrace_sidecar",
}
_TRIPLES = [
...
def _native_dep_label(cargo_label):
    ...
    if package in _SPECIAL_PATH_TARGETS:
        # Supplied by the native libddwaf/bindgen integration. Keeping these
        # as concrete labels makes tracer analysis fail until that real C++
        # library and generated Rust bindings are present.
        return _SPECIAL_PATH_TARGETS[package]
    if package not in _SPEC_BY_PACKAGE:
        fail("unmodeled reachable libdatadog path crate: %s" % cargo_label)
    return ":" + rust_workspace_target_name(package)
...
def libdatadog_native_features(package):
    """Returns exact target-selected Cargo features for a native path crate."""
    return _crate_features(_PRODUCT_DATA[package])
...
def _target_env(package):
    if package == "libdatadog/spawn_worker":
        return {
            "DD_SPAWN_LD_PRELOAD_TRAMPOLINE": "$(execpath :rust_spawn_worker_ld_preload_trampoline.shared_lib)",
            "DD_SPAWN_TRAMPOLINE": "$(execpath :rust_spawn_worker_trampoline)",
        }
...
def libdatadog_rust_targets():
    ...
    cc_binary(
        name = "rust_spawn_worker_ld_preload_trampoline.shared_lib",
    ...
    cc_library(
        name = "rust_spawn_worker_direct_entry",
    ...
    for crate in LIBDATADOG_CRATES:
        ...
        kwargs = {
            "name": name,
            "srcs": [_SOURCE_REPOSITORY + name + "_srcs"],
            "aliases": _native_aliases(package_data),
            "compile_data": [_SOURCE_REPOSITORY + name + "_data"],
            ...
            "deps": _resolved_deps(package_data) + ([":rust_libdatadog_crashtracker_emit_sicodes"] if crate.package == "libdatadog/libdd-crashtracker" else []),
        ...
        if crate.package == "libdatadog/spawn_worker":
            kwargs["compile_data"] = kwargs["compile_data"] + [
                ":rust_spawn_worker_ld_preload_trampoline.shared_lib",
                ":rust_spawn_worker_trampoline",
            ]
            kwargs["deps"] = kwargs["deps"] + [":rust_spawn_worker_direct_entry"]
