#!/bin/sh
# Validate and run the ASan-instrumented allocator smoke through only the
# declared glibc loader, ASan DSO, and target sysroot closure.
set -eu
...
scratch=${TMPDIR:-/tmp}/appsec-libxml2-asan.$$
mkdir -p "$scratch"
...
while IFS= read -r candidate; do
    ...
    while IFS= read -r needed; do
        ...
        IFS=$old_ifs
        test -n "$resolved" || { echo "undeclared libxml2 ASan dependency: $needed" >&2; exit 1; }
        printf '%s\n' "$resolved" >>"$scratch/queue"
    done <"$scratch/needed"
done <"$scratch/queue"
test "$found_asan" = 1
"$loader" --inhibit-cache --library-path "$library_path" "$binary"
printf '%s\n' 'AppSec libxml2 ASan allocator smoke passed' >"$marker"
