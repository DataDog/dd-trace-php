...
/**
 * Skip tests on older versions of PHP which could otherwise fail before
 * reaching SKIPIF.
 */
function sqreen_filter_tests(& $test_files)
{
    if (PHP_VERSION_ID < 50600) {
        function exclude_legacy_before_56($f) { return strpos($f, "-legacy.phpt") === false; }
        $test_files = array_filter($test_files, 'exclude_legacy_before_56');
        if (PHP_VERSION_ID <= 50303) {
            // PHP 5.3.3 cannot build mysqlnd shared (but later 5.3 versions can)
            function exclude_mysqlnd_in_533($f) {
                return strpos(file_get_contents($f), 'extension=mysqlnd.so') === false;
            }
            $test_files = array_filter($test_files, 'exclude_mysqlnd_in_533');
        }
    }
}
...
function main()
{
    ...
    if (empty($environment) && PHP_VERSION_ID >= 70100) {
        $environment = getenv();
    }
    ...
    if ($test_cnt) {
        sqreen_filter_tests($test_files);
        putenv('NO_INTERACTION=1');
...
";
        ...
        if (isset($section_text['REDIRECTTEST'])) {
            ...
            foreach (array('EXPECT', 'EXPECTF', 'EXPECTREGEX') as $prefix) {
                ...
                if (isset($section_text[$key])) {
                    ...
                    $output = system_with_timeout("$php_ -n -d display_errors=1 -d display_startup_errors=0 \"$temp_file_name\"", array());
                    $output = trim($output);
    ...
    if (array_key_exists('SKIPIF', $section_text)) {
        if (trim($section_text['SKIPIF'])) {
            ...
            $output = system_with_timeout("$extra $org_php $pass_options $extra_options -q $ini_settings $no_file_cache -d display_errors=1 -d display_startup_errors=0 \"$test_skipif\"", $env);
            $output = trim($output);
    ...
    if (array_key_exists('CLEAN', $section_text) && (!$no_clean || $cfg['keep']['clean'])) {
        if (trim($section_text['CLEAN'])) {
            ...
            if (!$no_clean) {
                $extra = !IS_WINDOWS ?
                    "unset REQUEST_METHOD; unset QUERY_STRING; unset PATH_TRANSLATED; unset SCRIPT_FILENAME; unset REQUEST_METHOD;" : "";
                system_with_timeout("$extra $php $pass_options $extra_options -q $ini_settings $no_file_cache \"$test_clean\"", $env);
            }
