...
mod tests {
    ...
    // The PHP extension omits rasp_rule entirely for regular (non-RASP) calls,
    // sending an empty options map. Verify this deserializes without error.
    #[test]
    fn test_request_exec_options_empty_map() {
        let bytes: &[u8] = &[0x80]; // msgpack fixmap, 0 elements: {}
        let result: Result<RequestExecOptions, _> = rmp_serde::from_slice(bytes);
        assert!(result.is_ok(), "deserialization failed: {:?}", result);
        let opts = result.unwrap();
        assert_eq!(opts.run_type, WafRunType::NonRasp);
        assert_eq!(opts.subctx_id, None);
        assert!(!opts.subctx_last_call);
    }
