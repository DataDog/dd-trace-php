...
// pointer to ddtrace_sidecar on ddtrace side
static ddog_SidecarTransport **nullable _ddtrace_sidecar;
...
#define ASSIGN_DLSYM(var, export)                                              \
    ...
    ASSIGN_DLSYM(_ddtrace_sidecar, "ddtrace_sidecar");
    ...
    uint64_t client_id, const uint8_t *nonnull request, size_t request_len)
{
    ...
    if (!_ddtrace_sidecar || !_ddog_sidecar_send_appsec_message ||
    ...
    return _ddog_sidecar_send_appsec_message(
        _ddtrace_sidecar, session_id_slice, client_id, request_slice);
}
