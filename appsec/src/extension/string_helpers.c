// Unless explicitly stated otherwise all files in this repository are
// dual-licensed under the Apache-2.0 License or BSD-3-Clause License.
//
// This product includes software developed at Datadog
// (https://www.datadoghq.com/). Copyright 2021 Datadog, Inc.

#include "string_helpers.h"

static const unsigned char header_lc_dash[256] = {};

static const unsigned char lc_table[256] = {};

bool dd_string_starts_with_lc(const char *nonnull s, size_t len,
    const char *nonnull cmp_lc, size_t cmp_lc_len)
{
    if (len < cmp_lc_len) {
        return false;
    }
    const char *const end = cmp_lc + cmp_lc_len;
    while (cmp_lc < end) {
        if (*cmp_lc++ != (char)lc_table[(unsigned char)*s++]) {
            return false;
        }
    }
    return true;
}

bool dd_string_equals_lc(const char *nonnull s, size_t len,
    const char *nonnull cmp_lc, size_t cmp_lc_len)
{
    if (len != cmp_lc_len) {
        return false;
    }
    const char *const end = cmp_lc + cmp_lc_len;
    while (cmp_lc < end) {
        if (*cmp_lc++ != (char)lc_table[(unsigned char)*s++]) {
            return false;
        }
    }
    return true;
}

void dd_string_normalize_header(char *nonnull s, size_t len)
{
    // in and out can overlap
    const char *end = s + len;
    for (char *p = s; p != end; p++) {
        *p = (char)header_lc_dash[(unsigned char)*p];
    }
}
void dd_string_normalize_header2(
    const char *nonnull in, char *nonnull out, size_t len)
{
    // in and out can overlap
    const char *end = in + len;
    for (const char *p = in; p != end; p++) {
        unsigned char c = *p;
        *out++ = (char)header_lc_dash[c];
    }
    *out = '\0';
}
