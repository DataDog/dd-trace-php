...
struct identity_hash {
    std::uint64_t operator()(std::uint64_t number) const { return number; }
};
...
class timed_set {
    ...
    struct table {
        ...
        // higher load factors.
        std::pair<entry &, bool> find_slot(std::uint64_t number)
        {
            ...
            do {
                ...
                idx = (idx + 1) % Capacity;
            } while (__builtin_expect(idx != orig_idx, 1));
...
private:
    void rebuild_table(table &old_table, std::uint32_t report_threshold)
    {
        ...
        for (std::size_t slot = 0; slot < Capacity; ++slot) {
            ...
            if (entry.key.load() != 0 &&
                entry.data.load().last_reported >= report_threshold) {
                heap.push(copiable_entry{entry.key.load(), entry.data.load()});
            }
        }
        ...
        while (!heap.empty() && count < MaxItems * 2 / 3) {
            ...
            new_entry.key.store(entry.key);
            new_entry.data.store(entry.data);
            heap.pop();
            count++;
        }
        new_table->size.store(count);

        // swap tables
        rcu.update(std::move(new_table));
        rebuild_in_progress.store(false);
    }
