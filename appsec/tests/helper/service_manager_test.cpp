...
TEST(ServiceManagerTest, LoadRulesOK)
{
    ...
    auto service = manager.get_or_create_service(engine_settings, {}, {});
    ...
    auto service2 = manager.get_or_create_service(engine_settings, {}, {});
    EXPECT_EQ(manager.get_cache().size(), 1);
    ...
    auto service3 = manager.get_or_create_service(engine_settings, {true}, {});
    ASSERT_NE(service3.get(), service_rp);
    ASSERT_TRUE(weak_ptr.expired());
    EXPECT_EQ(manager.get_cache().size(), 1);

    // another service identifier should result in another service
    auto service4 = manager.get_or_create_service(engine_settings, {}, {});
    ...
    EXPECT_THROW(
        {
            ...
            engine_settings.waf_timeout_us = 42;
            manager.get_or_create_service(engine_settings, {}, {});
        },
        std::runtime_error);
}
...
TEST(ServiceManagerTest, UniqueServices)
{
    ...
    engine_settings.rules_file = fn;
    engine_settings.waf_timeout_us = 42;
    auto service1 = manager.get_or_create_service(engine_settings, {}, {});
    auto service2 = manager.get_or_create_service(engine_settings, {}, {});
