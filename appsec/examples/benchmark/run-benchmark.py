...
OTEL_PROFILER_RUNTIME_IMAGE = "ubuntu:24.04"
OTEL_PROFILER_CONTAINER_BIN = "/usr/local/bin/ebpf-profiler"
CPU_GOVERNOR_ROOT = Path("/sys/devices/system/cpu/cpufreq")
...
def main() -> int:
    ...
    if "flamegraph" in cases:
        validate_flamegraph_profiler_args(args)
    ensure_docker_image("grafana/k6")
    if "memory-profiler" in cases:
        ensure_docker_image(args.memory_profiler_image)
    if "flamegraph" in cases and args.flamegraph_profiler_backend == "docker":
        ensure_docker_image(args.flamegraph_profiler_image)
    ...
    governor_state = configure_cpu_governor(
        args.cpu_governor,
        args.cpu_governor_write_command,
        runtime.out / "cpu-governor.json",
    )
    ...
    try:
        ...
        try:
            ...
            restore_cpu_governor(governor_state)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cpu-governor",
        choices=("performance", "none"),
        default="performance",
        help="Linux CPU governor to use during the benchmark, or none to leave it unchanged.",
    )
    parser.add_argument(
        "--cpu-governor-write-command",
        default=default_cpu_governor_write_command(),
        help=(
            "Command with tee-compatible stdin/path arguments used to update CPU "
            "governor files."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--flamegraph-profiler-backend",
        choices=("docker", "native"),
        default="docker",
        help="Run the OTel profiler in a runner-managed privileged container or as a native command.",
    )
    parser.add_argument(
        "--flamegraph-profiler-bin",
        default=str(default_profiler_bin()),
        help="Host ebpf-profiler binary bind-mounted by the Docker backend.",
    )
    parser.add_argument(
        "--flamegraph-profiler-image",
        default=OTEL_PROFILER_RUNTIME_IMAGE,
        help="Runtime image used by the Docker profiler backend.",
    )
    parser.add_argument(
        "--flamegraph-profiler-command",
        default=f"sudo -n {default_profiler_bin()}",
        help="Profiler command used by the native backend.",
    )
    parser.add_argument("--flamegraph-profile-receiver-command", default=str(default_profile_receiver_bin()))
    parser.add_argument(
        "--flamegraph-profiler-stop-command",
        help="Optional additional cleanup command, primarily for externally managed native profilers.",
    )
...
def validate_flamegraph_profiler_args(args: argparse.Namespace) -> None:
    if args.flamegraph_profiler_backend == "docker":
        profiler_bin = Path(args.flamegraph_profiler_bin).expanduser().resolve()
        if not profiler_bin.is_file():
            raise SystemExit(
                f"--flamegraph-profiler-bin is not a file: {profiler_bin}"
            )
        if not os.access(profiler_bin, os.X_OK):
            raise SystemExit(
                f"--flamegraph-profiler-bin is not executable: {profiler_bin}"
            )
        if not args.flamegraph_profiler_image.strip():
            raise SystemExit("--flamegraph-profiler-image cannot be empty")
        args.flamegraph_profiler_bin = str(profiler_bin)
        return
...
def default_cpu_governor_write_command() -> str:
    return "tee" if os.geteuid() == 0 else "sudo -n tee"
...
def configure_cpu_governor(
    ...
    write_command: str,
    report_path: Path,
    governor_root: Path = CPU_GOVERNOR_ROOT,
) -> CpuGovernorState:
    paths = cpu_governor_paths(governor_root)
    original = {path: read_cpu_governor(path) for path in paths}
    state = CpuGovernorState(requested, write_command, original, report_path)

    if requested == "none":
        write_cpu_governor_report(state, "disabled", original)
        return state
    if not paths:
        write_cpu_governor_report(state, "unavailable", {})
        print("CPU governor control unavailable: no Linux cpufreq policies found", flush=True)
        return state

    try:
        write_cpu_governor_values(paths, requested, write_command)
        effective = {path: read_cpu_governor(path) for path in paths}
        mismatched = [path for path, value in effective.items() if value != requested]
        if mismatched:
            raise RuntimeError(
                f"CPU governor did not change to {requested!r} for: "
                + ", ".join(str(path) for path in mismatched)
            )
        write_cpu_governor_report(state, "configured", effective)
        print(f"Configured {len(paths)} CPU policies with governor {requested!r}", flush=True)
        return state
    except BaseException as error:
        restore_error = restore_cpu_governor_values(original, write_command)
        write_cpu_governor_report(
            state,
            "configuration-failed",
            read_cpu_governors(paths),
            error=str(error),
            restore_error=restore_error,
        )
        if restore_error:
            error.add_note(f"CPU governor rollback also failed: {restore_error}")
        raise


def restore_cpu_governor(state: CpuGovernorState) -> None:
    if state.requested == "none" or not state.original:
        return

    restore_error = restore_cpu_governor_values(state.original, state.write_command)
    effective = read_cpu_governors(list(state.original))
    write_cpu_governor_report(
        state,
        "restore-failed" if restore_error else "restored",
        effective,
        restore_error=restore_error,
    )
    if restore_error:
        raise RuntimeError(f"failed to restore CPU governors: {restore_error}")
    print(f"Restored {len(state.original)} CPU policy governors", flush=True)


def cpu_governor_paths(governor_root: Path) -> list[Path]:
    return sorted(governor_root.glob("policy[0-9]*/scaling_governor"))


def read_cpu_governor(path: Path) -> str:
    return path.read_text().strip()


def read_cpu_governors(paths: list[Path]) -> dict[Path, str]:
    values: dict[Path, str] = {}
    for path in paths:
        try:
            values[path] = read_cpu_governor(path)
        except OSError:
            values[path] = "<unreadable>"
    return values


def write_cpu_governor_values(paths: list[Path], value: str, write_command: str) -> None:
    command = shlex.split(write_command)
    if not command:
        raise RuntimeError("--cpu-governor-write-command cannot be empty")
    if shutil.which(command[0]) is None:
        raise RuntimeError(f"CPU governor write command not found: {command[0]}")

    print(
        "+ " + " ".join(shlex.quote(part) for part in command)
        + f" ({len(paths)} CPU governor files)",
        flush=True,
    )
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"could not set CPU governor to {value!r}: {detail}")


def restore_cpu_governor_values(
    original: dict[Path, str],
    write_command: str,
) -> str | None:
    paths_by_value: dict[str, list[Path]] = defaultdict(list)
    for path, value in original.items():
        paths_by_value[value].append(path)

    errors: list[str] = []
    for value, paths in paths_by_value.items():
        try:
            write_cpu_governor_values(paths, value, write_command)
        except RuntimeError as error:
            errors.append(str(error))
    if errors:
        return "; ".join(errors)

    effective = read_cpu_governors(list(original))
    mismatched = [
        path for path, value in effective.items()
        if value != original[path]
    ]
    if mismatched:
        return "restored governor did not match original for: " + ", ".join(
            str(path) for path in mismatched
        )
    return None


def write_cpu_governor_report(
    state: CpuGovernorState,
    status: str,
    ...
    *,
    ...
    phase = "restored" if status in {"restored", "restore-failed"} else "configured"
    report.update(
        {
            "error": error,
            "original": {str(path): value for path, value in state.original.items()},
    ...
    report[phase] = {str(path): value for path, value in effective.items()}
    write_text(state.report_path, json.dumps(report, indent=2, sort_keys=True) + "\n")
...
def prepare_flamegraph_tools(args: argparse.Namespace) -> int:
    ...
    print()
    print("The default Docker backend will bind-mount the profiler above.")
    print("To use a native profiler command instead, pass:")
    print("  --flamegraph-profiler-backend native")
...
def retry_appsec_ready_response(
    runtime: Runtime,
    timeout: float,
) -> subprocess.CompletedProcess[str]:
    deadline = time.monotonic() + timeout
    while True:
        response = appsec_ready_response(
            runtime,
            timeout=deadline - time.monotonic(),
        )
        if response.returncode == 0 and response.stdout == "204":
            return response
...
def appsec_readiness_check(
    ...
    *,
    ...
    if args.server != "fpm":
        response = retry_appsec_ready_response(
            runtime,
...
def run_case(
    ...
    ready_before = retry_appsec_ready_response(runtime, args.ready_timeout)
    validation = {
    ...
    try:
        ...
        ready_response = retry_appsec_ready_response(runtime, args.ready_timeout)
...
def run_flamegraph_case(
        ...
        json.dumps(
            {
                ...
                "profiler": flamegraph_profiler_config(args, runtime),
    ...
    try:
        ...
        profiler = start_flamegraph_profiler(args, runtime, case_dir)
        ...
        if profiler is not None:
            stop_flamegraph_profiler(profiler, case_dir / "profiler.stop.log")
    ...
    ready_after = retry_appsec_ready_response(runtime, args.ready_timeout)
    helper_shutdown = "AppSec helper shutdown initiated" in helper_log
    open_file_limit_error = bool(
...
def flamegraph_profiler_options(args: argparse.Namespace) -> list[str]:
    return [
...
def flamegraph_profiler_container_name(runtime: Runtime) -> str:
    return f"{runtime.project}-otel-profiler"


def flamegraph_profiler_config(
    args: argparse.Namespace,
    runtime: Runtime,
) -> dict[str, Any]:
    config: dict[str, Any] = {
        "backend": args.flamegraph_profiler_backend,
        "options": flamegraph_profiler_options(args),
    }
    if args.flamegraph_profiler_backend == "docker":
        profiler_bin = Path(args.flamegraph_profiler_bin)
        config.update(
            {
                "binary_path": str(profiler_bin),
                "binary_sha256": sha256_file(profiler_bin),
                "container_image": args.flamegraph_profiler_image,
                "container_name": flamegraph_profiler_container_name(runtime),
        ...
        config["command"] = shlex.split(args.flamegraph_profiler_command)
    return config


def start_flamegraph_profiler(
    args: argparse.Namespace,
    runtime: Runtime,
    case_dir: Path,
) -> FlamegraphProfiler:
    ...
    if args.flamegraph_profiler_backend == "docker":
        ...
        if docker_container_exists(container_name):
            raise RuntimeError(
                f"OTel profiler container already exists: {container_name}"
            )
        tracefs_and_exec = (
            'if ! grep -qs " /sys/kernel/tracing tracefs " /proc/mounts; then\n'
            "  mount -t tracefs tracefs /sys/kernel/tracing\n"
            "fi\n"
            f'exec {OTEL_PROFILER_CONTAINER_BIN} "$@"'
        )
        cmd = [
            "docker",
            "run",
            ...
            "-v",
            (
                f"{args.flamegraph_profiler_bin}:"
                f"{OTEL_PROFILER_CONTAINER_BIN}:ro"
            ),
            args.flamegraph_profiler_image,
            "sh",
            "-ceu",
            tracefs_and_exec,
            "otel-profiler",
        ...
        cmd = [*shlex.split(args.flamegraph_profiler_command), *options]
...
def stop_flamegraph_profiler(
    profiler: FlamegraphProfiler,
    log_path: Path,
) -> None:
    if profiler.backend == "native":
        terminate_process_group(profiler.process, "profiler", log_path)
        return

    container_name = profiler.container_name
    if container_name is None:
        raise RuntimeError("Docker profiler has no container name")

    messages: list[str] = []
    if docker_container_exists(container_name):
        stop_command = ["docker", "stop", "--timeout", "10", container_name]
        result = run(
            stop_command,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
        ...
        messages.append(
            f"docker stop exited {result.returncode}: {result.stdout.strip()}"
        )

    try:
        profiler.process.wait(timeout=15)
    except subprocess.TimeoutExpired:
        messages.append("docker profiler client did not exit after docker stop")
    ...
    if docker_container_exists(container_name):
        ...
        messages.append(
            f"docker rm -f exited {result.returncode}: {result.stdout.strip()}"
        )

    if profiler.process.poll() is None:
        terminate_process_group(
            profiler.process,
            "docker profiler client",
            log_path,
        )
        return

    if not messages:
        messages.append("Docker profiler had already exited and been removed")
    write_text(log_path, "\n".join(messages) + "\n")
...
def extract_flamegraph_symbols(case_dir: Path, container_id: str) -> dict[str, Path]:
    ...
    queries = {
        ...
        "datadog-profiling.so": "ext_dir=$(php -r 'echo ini_get(\"extension_dir\");'); test -f \"$ext_dir/datadog-profiling.so\" && echo \"$ext_dir/datadog-profiling.so\" || true",
