...
New-ItemProperty -Path $localDumps -Name DumpCount -Value 20 -PropertyType DWord -Force | Out-Null
...
# disables WER. Change only this test's FILE process; keep normal failure/exit
# handling and avoid loading diagnostic extensions into the crashing process.
...
$replacement = "'suppress_errors' => strpos(`$commandline, 'rc_process_tags.php') === false"
