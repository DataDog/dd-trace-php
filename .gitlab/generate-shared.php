...
"Zend Abstract Interface Tests: [<?= $major_minor ?>, <?= $switch_php_version ?>]":
  ...
  script:
    ...
    - CMAKE_PREFIX_PATH=/opt/catch2 Tea_ROOT=/opt/tea/<?= $switch_php_version ?> cmake <?= $toolchain ?> -DCMAKE_BUILD_TYPE=Debug -DBUILD_ZAI_TESTING=ON -DPhpConfig_ROOT=$(php-config --prefix) ../../zend_abstract_interface
    - make -j all
    - make test
    - grep -e "=== Total [0-9]+ memory leaks detected ===" Testing/Temporary/LastTest.log && exit 1 || true
<?php
...
"Extension Tea Tests: [<?= $major_minor ?>, <?= $switch_php_version ?>]":
  ...
  script:
    ...
    - CMAKE_PREFIX_PATH=/opt/catch2 Tea_ROOT=/opt/tea/<?= $switch_php_version ?> cmake <?= $toolchain ?> -DCMAKE_BUILD_TYPE=Debug -S ../../tests/tea
