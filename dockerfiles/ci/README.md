...
* **Pipeline generation:** `.gitlab/generate-ci-images.php` reads those compose
  ...
  The hand-written parts — job templates and the Windows jobs — live in
  `.gitlab/ci-images.static.yml`. The generator runs inside the
  `generate-templates` job; the manual `ci-images` job (stage `ci-build`)
  launches the generated child pipeline.
