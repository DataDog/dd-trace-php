...
if [[ "${1:-}" == "--docker" ]]; then
  ...
  [[ -f docker-compose.linux.override.yml ]] && COMPOSE_FILES="${COMPOSE_FILES} -f docker-compose.linux.override.yml"
  ...
  exec docker compose ${COMPOSE_FILES} run --rm --no-deps \
    ...
    8.5-bookworm /home/circleci/app/dockerfiles/dev/run_zai_tests_linux.sh
fi
