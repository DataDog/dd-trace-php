...
static inline void _dd_es_initialize_defered_integration() {
    if (!ddtrace_config_integration_enabled(DDTRACE_STRING_LITERAL("elasticsearch"))) {
        return;
    }

    static struct ddtrace_defered_integration _es[] = {
    ...
    dd_load_defered_integration_list(_es, SIZE_OF_DEFERED_LIST(_es));
}
