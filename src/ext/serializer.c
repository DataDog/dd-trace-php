...
#define ADD_ELEMENT_IF_PROP_TYPE(name, type)     \
    do {        \
        ...
        if (Z_TYPE_P(prop) == (type)) { \
            add_assoc_zval(serialized, (name), prop); \
        } \
    } while (0);
...
void ddtrace_serialize_span_to_array(ddtrace_span_stack_t *stack, zval *serialized TSRMLS_DC) {
    ...
    add_assoc_long(serialized, "trace_id", DDTRACE_G(root_span_id));
    add_assoc_long(serialized, "span_id", stack->span_id);
    if (stack->parent_id > 0) {
        add_assoc_long(serialized, "parent_id", stack->parent_id);
    }
    add_assoc_long(serialized, "start", stack->start);
    add_assoc_long(serialized, "duration", stack->duration);
...
void ddtrace_serialize_span_stack_to_array(zval *retval TSRMLS_DC) {
    ...
    while (stack != NULL) {
        add_next_index_zval(retval, stack->span);
        stack = stack->next;
    }
}
