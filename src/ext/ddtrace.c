...
PHP_INI_BEGIN()
STD_PHP_INI_ENTRY("ddtrace.disable", "0", PHP_INI_SYSTEM, OnUpdateBool, disable,
...
static PHP_RINIT_FUNCTION(ddtrace) {
    ...
    zend_hash_init(&DDTRACE_G(function_lookup), 8, NULL, (dtor_func_t)ddtrace_class_lookup_free,
                   0);
...
static PHP_MINFO_FUNCTION(ddtrace) {
    ...
    php_info_print_table_header(2, "Datadog tracing support",
                                DDTRACE_G(disable) ? "disabled" : "enabled");
...
static PHP_FUNCTION(dd_trace) {
    ...
    if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "Csz", &clazz,
                                 &Z_STRVAL_P(function), &Z_STRLEN_P(function),
                                 &callable) != SUCCESS &&
        zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "sz",
                                 &Z_STRVAL_P(function), &Z_STRLEN_P(function),
                                 &callable) != SUCCESS) {
...
#else
    if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), "CSz", &clazz,
                                 &function, &callable) != SUCCESS &&
        zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), "Sz", &function,
                                 &callable) != SUCCESS) {
        zend_throw_exception_ex(
...
zend_module_entry ddtrace_module_entry = {
    ...
    PHP_DDTRACE_VERSION,    STANDARD_MODULE_PROPERTIES};
