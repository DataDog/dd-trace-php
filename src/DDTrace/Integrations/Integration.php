...
abstract class Integration implements \DDTrace\Integration
{
    ...
    {
        $name = $this->getName();
        if (\DDTrace\Config\integration_analytics_enabled($name) || $this->isForcingTraceAnalytics()) {
            $span->metrics[Tag::ANALYTICS_KEY] = \DDTrace\Config\integration_analytics_sample_rate($name);
        }
    }

    /**
     * Whether this integration trace analytics configuration is enabled.
     *
    ...
    {
        return \dd_trace_env_config("DD_TRACE_ANALYTICS_ENABLED");
    }
