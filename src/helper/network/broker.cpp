...
// NOLINTNEXTLINE
bool default_reference_func(msgpack::type::object_type /*type*/, std::size_t /*len*/, void*) {
    return true;
}
} // namespace
...
request broker::recv(std::chrono::milliseconds initial_timeout) const
{
    ...
    header_t h;
    // NOLINTNEXTLINE
    std::size_t res = socket_->recv(reinterpret_cast<char *>(&h), sizeof(header_t));
    if (res != sizeof(header_t)) {
        // The sender probably closed the socket
        throw std::length_error("Not enough data for header:" +
    ...
    if (h.size >= max_msg_body_size) {
        throw std::length_error("Message body too large: " + std::to_string(h.size));
    }
    ...
    if (res != h.size) {
        throw std::length_error("Not enough data for message body:" +
            std::to_string(res) + " bytes, required " + std::to_string (h.size) +
            " bytes");
    }
