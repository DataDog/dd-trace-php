...
typedef struct ddog_MockServer ddog_MockServer;
...
typedef enum ddog_Option_ArcOwnedFd_Tag {
  DDOG_OPTION_ARC_OWNED_FD_SOME_ARC_OWNED_FD,
  DDOG_OPTION_ARC_OWNED_FD_NONE_ARC_OWNED_FD,
} ddog_Option_ArcOwnedFd_Tag;

typedef struct ddog_Arc_OwnedFd {} ddog_Arc_OwnedFd;

typedef struct ddog_Option_ArcOwnedFd {
  ddog_Option_ArcOwnedFd_Tag tag;
  union {
    struct {
      struct ddog_Arc_OwnedFd some;
    };
  };
} ddog_Option_ArcOwnedFd;
...
typedef struct ddog_PlatformHandle_File {
  int fd;
  struct ddog_Option_ArcOwnedFd inner;
 ...
 * allowing safe transfer and sharing of file handles across processes, and threads
 */
typedef struct ddog_PlatformHandle_UnixStream {
  int fd;
  struct ddog_Option_ArcOwnedFd inner;
} ddog_PlatformHandle_UnixStream;

typedef struct ddog_NativeUnixStream {
  struct ddog_PlatformHandle_UnixStream handle;
