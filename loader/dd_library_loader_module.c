...
static void ddloader_info_function(zend_module_entry *module) {
    ...
    for (unsigned int i = 0; i < EXT_COUNT; ++i) {
        ...
        php_info_print_table_row(2, "Extra config", ddloader_injected_ext_config[i].extra_config ? ddloader_injected_ext_config[i].extra_config : "");
        php_info_print_table_row(2, "Logs", ddloader_injected_ext_config[i].logs ? ddloader_injected_ext_config[i].logs : "");
        php_info_print_table_end();
    }
}
