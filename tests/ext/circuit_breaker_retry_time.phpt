...
echo '1 second retry set CAN_RETRY ' . (dd_tracer_circuit_breaker_can_try() ? 'true' : 'false') . PHP_EOL; // 1 second not yet passed so no retry for you
sleep(1);
echo '1 second has passed CAN_RETRY ' . (dd_tracer_circuit_breaker_can_try() ? 'true' : 'false') . PHP_EOL; // 1 second passed lets retry

dd_tracer_circuit_breaker_register_error();
echo '1 second has passed but new error was registered CAN_RETRY ' . (dd_tracer_circuit_breaker_can_try() ? 'true' : 'false') . PHP_EOL; // start counting from beginning
sleep(1);
echo 'another 1 second has passed CAN_RETRY ' . (dd_tracer_circuit_breaker_can_try() ? 'true' : 'false') . PHP_EOL; // another 1 second passed lets retry
?>
--EXPECT--
