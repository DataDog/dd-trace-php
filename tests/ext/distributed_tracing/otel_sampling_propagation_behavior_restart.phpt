--TEST--
OpenTelemetry sampling state is regenerated when extraction restarts the trace
--ENV--
...
DDTrace\consume_distributed_tracing_headers([
    ...
    'tracestate' => "ot=rv:$inboundRandomValue;th:e6666666666668;foo:bar",
]);
...
echo 'inbound unknown removed: ', strpos($ot, 'foo:bar') === false ? 'yes' : 'no', PHP_EOL;
echo 'link retains unknown: ', strpos(DDTrace\root_span()->links[0]->traceState, 'foo:bar') !== false ? 'yes' : 'no', PHP_EOL;
...
inbound unknown removed: yes
link retains unknown: yes
