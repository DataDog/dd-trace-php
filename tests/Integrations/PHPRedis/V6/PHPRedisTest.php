...
class PHPRedisTest extends IntegrationTestCase
{
    ...
    public function testRedisInExtendedClass()
    {
        ...
        $this->assertFlameGraph($traces, [
            SpanAssertion::build(
                'DDTrace\Tests\Integrations\PHPRedis\V6\CustomRedisClass.connect',
                'phpredis',
                'redis',
                'DDTrace\Tests\Integrations\PHPRedis\V6\CustomRedisClass.connect'
            )->withExactTags([
    ...
    public function testListFunctions($method, $args, $expectedResult, $expectedFinal, $rawCommand)
    {
        ...
        foreach ($expectedFinal as $list => $values) {
            ...
            for ($element = 0; $element < count($values); $element++) {
                $this->assertSame($expectedFinal[$list][$element], $this->redis->lIndex($list, $element));
            }
        }
    }
    ...
    public function testSortedSetFunctions($method, $args, $expectedResult, $expectedFinal, $rawCommand)
    {
        ...
        foreach ($expectedFinal as $set => $value) {
            $this->assertSame($this->redis->zCard($set), $value);
        }
    }
    ...
    public function testRename()
    {
        ...
        $this->assertFlameGraph($traces, [
            SpanAssertion::build(
                ...
                'redis',
                "Redis.rename"
            )->withExactTags(['redis.raw_command' => "rename k1 k2", Tag::SPAN_KIND => 'client',
                Tag::COMPONENT => 'phpredis', Tag::DB_SYSTEM => 'redis', Tag::TARGET_HOST => $this->host]),
        ]);
