...
TEST(EngineTest, MultipleSubscriptors) {
    ...
    EXPECT_CALL(*sub1, get_subscriptions())
        .WillRepeatedly(Return(std::vector<std::string_view>{"a", "b", "e", "f"}));
    EXPECT_CALL(*sub1, get_listener())
        .WillRepeatedly(Return(blocker));

    mock::subscriber::ptr sub2 = mock::subscriber::ptr(new mock::subscriber());
    EXPECT_CALL(*sub2, get_subscriptions())
        .WillRepeatedly(Return(std::vector<std::string_view>{"c", "d", "e", "g"}));
    EXPECT_CALL(*sub2, get_listener())
        .WillRepeatedly(Return(recorder));
    ...
    EXPECT_CALL(*sub3, get_listener())
        .WillRepeatedly(Return(ignorer));
...
TEST(EngineTest, StatefulSubscriptor) {
    ...
    mock::subscriber::ptr sub = mock::subscriber::ptr(new mock::subscriber());
    EXPECT_CALL(*sub, get_subscriptions())
        .WillRepeatedly(Return(std::vector<std::string_view>{"sub1", "sub2", "final"}));
    EXPECT_CALL(*sub, get_listener())
        .WillRepeatedly(Return(listener));
