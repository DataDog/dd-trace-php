...
echo "GET http://localhost" | vegeta attack -format=http -duration=${DURATION:-30s} -keepalive=true -max-workers=5 -rate=0 | tee results.bin | vegeta report --type=json --output=/results/results.json
echo "Done loading"
