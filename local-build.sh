...
function build_version() {
    ...
    SUFFIX=${3:-''}
    rm -rf tmp
    docker run --rm -w /var/app -v $(pwd):/var/app circleci/php:${PHP_VERSION}${SUFFIX} make all CFLAGS="-g"
...
function build_version_54() {
    ...
    PHP_API=20100412
    rm -rf tmp
    docker run --rm -w /var/app -v $(pwd):/var/app datadog/docker-library:ddtrace_centos_6_php_5_4 make all CFLAGS="-O2 -Wall -Wextra" ECHO_ARG="-e"
