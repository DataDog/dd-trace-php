...
LIBS_PATH = '/'
p = re.compile(r"datadog-php-tracer.*deb$")
...
for path, sub_dirs, files in os.walk(LIBS_PATH):
    for name in files:
      if p.match(name):
        ...
        transfer.upload_file(os.path.join(path, name), S3_BUCKET_NAME, f'php/{name}')
        ...
        with tempfile.NamedTemporaryFile(mode='w') as fp:
          for line in [os.getenv('CIRCLE_BRANCH'), os.getenv('CIRCLE_SHA1'), name, os.getenv('CIRCLE_USERNAME')]:
            fp.write(f'{line}\n')
          fp.seek(0)
          transfer.upload_file(fp.name, S3_BUCKET_NAME, 'php/index.txt')
