...
static int64_t dd_init_live_debugger_probe(const ddog_Probe *probe, dd_probe_def *def, zai_hook_begin begin, zai_hook_end end, void (*def_dtor)(void *), size_t dynamic) {
    ...
    if (target->type_name.len) {
        if (!ddog_type_can_be_instrumented(DATADOG_G(remote_config_state), target->type_name)) {
...
static void dd_log_probe_ensure_payload(dd_log_probe_dyn *dyn, dd_log_probe_def *def, ddog_CharSlice *msg) {
    if (dyn->payload) {
        ...
        dyn->payload = ddog_create_log_probe_snapshot(
            ...
            DDOG_CHARSLICE_C("php"),
            ddtrace_nanoseconds_realtime() / 1000000,
            dd_zend_string_to_CharSlice(datadog_process_tags_get_serialized()));
    }
}
...
static void dd_metric_probe_end(zend_ulong invocation, zend_execute_data *execute_data, zval *retval, void *auxiliary, void *dynamic) {
    ...
    switch (def->probe.probe.metric.kind) {
        case DDOG_METRIC_KIND_COUNT:
            datadog_sidecar_dogstatsd_count(metric_name, (zend_long)metric_value, NULL);
            break;
        case DDOG_METRIC_KIND_GAUGE:
            datadog_sidecar_dogstatsd_gauge(metric_name, metric_value, NULL);
            break;
        case DDOG_METRIC_KIND_HISTOGRAM:
            datadog_sidecar_dogstatsd_histogram(metric_name, metric_value, NULL);
            break;
        case DDOG_METRIC_KIND_DISTRIBUTION:
            datadog_sidecar_dogstatsd_distribution(metric_name, metric_value, NULL);
            break;
    }
...
static void dd_remove_live_debugger_probe(int64_t id) {
    dd_probe_def *def;
    if ((def = zend_hash_index_find_ptr(&DDTRACE_G(active_live_debugger_hooks), (zend_ulong)id))) {
...
static uintptr_t dd_eval_length(void *ctx, const void *zvp) {
    ...
    switch (Z_TYPE_P(zv)) {
        ...
        case IS_DOUBLE:
        case IS_LONG: ;
            zend_string *str = datadog_convert_to_str(zv);
...
void ddtrace_live_debugger_rinit(void) {
    zend_hash_init(&DDTRACE_G(active_live_debugger_hooks), 8, NULL, NULL, 0);
}
...
bool ddtrace_alter_dynamic_instrumentation_config(zval *old_value, zval *new_value, zend_string *new_str) {
    ...
    if (DATADOG_G(remote_config_state) && !DATADOG_G(remote_config_writing) && !ddog_remote_config_alter_dynamic_config(DATADOG_G(remote_config_state), DDOG_CHARSLICE_C("datadog.dynamic_instrumentation.enabled"), zend_string_copy(new_str))) {
        return false;
    }

    // Apply/unapply probe hooks based on the new DI enabled state.
    if (DATADOG_G(remote_config_state)) {
        ddog_set_dynamic_instrumentation_enabled(DATADOG_G(remote_config_state), enabled);
    }
...
ddog_DynamicInstrumentationConfigState ddtrace_dynamic_instrumentation_state(void) {
    if (zai_config_is_modified(DATADOG_CONFIG_DD_DYNAMIC_INSTRUMENTATION_ENABLED)) {
