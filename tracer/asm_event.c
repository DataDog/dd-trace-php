...
DATADOG_PUBLIC void ddtrace_emit_asm_event() {
    ...
    if (DDTRACE_G(active_stack)->root_span) {
        ...
        DATADOG_G(asm_event_emitted) = true;
    }
...
bool ddtrace_asm_event_emitted() {
    return DATADOG_G(asm_event_emitted) || (DDTRACE_G(active_stack) && DDTRACE_G(active_stack)->root_span && DDTRACE_G(active_stack)->root_span->asm_event_emitted);
}

void ddtrace_asm_event_rinit() {
    DATADOG_G(asm_event_emitted) = false;
}
