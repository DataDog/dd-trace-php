...
static zend_string *dd_tag_p_ts;

void ddtrace_trace_source_minit() {
    dd_tag_p_ts = zend_string_init_interned(ZEND_STRL(DD_P_TS_KEY), 1 /* permanent */);
}
...
static void ddtrace_trace_source_add_propagated_tag(zend_string *encoded) {
    ...
    ddtrace_add_propagated_tag(dd_tag_p_ts, &ts_encoded_zv);
