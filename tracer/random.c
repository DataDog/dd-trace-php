...
datadog_trace_id ddtrace_parse_userland_trace_id(zend_string *tid) {
    datadog_trace_id num = {0};
    const char *id = ZSTR_VAL(tid);
    for (size_t i = 0; i < ZSTR_LEN(tid); i++) {
        if (id[i] < '0' || id[i] > '9') {
            return (datadog_trace_id){ 0 };
        }
...
datadog_trace_id ddtrace_parse_hex_trace_id(char *trace_id, ssize_t trace_id_len) {
    return (datadog_trace_id){
...
datadog_trace_id ddtrace_peek_trace_id(void) {
    ddtrace_span_properties *pspan = DDTRACE_G(active_stack) ? DDTRACE_G(active_stack)->active : NULL;
    return pspan ? SPANDATA(pspan)->root->trace_id : DDTRACE_G(distributed_trace_id);
}

int ddtrace_conv10_trace_id(datadog_trace_id id, uint8_t reverse[DD_TRACE_MAX_ID_LEN]) {
    reverse[0] = 0;
    int i = 0;
    while (id.high) {
