...
static HashTable dd_string_to_integration_name_map;

static void dd_add_integration_to_map(char* name, size_t name_len, ddtrace_integration* integration);

void ddtrace_integrations_mshutdown(void) { zend_hash_destroy(&dd_string_to_integration_name_map); }
...
static void dd_invoke_integration_loader_and_unhook_posthook(zend_ulong invocation, zend_execute_data *execute_data, zval *retval, void *auxiliary, void *dynamic) {
    ...
    if (aux->name == -1u || ddtrace_integrations[aux->name].is_enabled()) {
        if (aux->name != -1u) {
...
void ddtrace_integrations_minit(void) {
    zend_hash_init(&dd_string_to_integration_name_map, ddtrace_integrations_len, NULL, NULL, 1);
    ...
    for (size_t i = 0; i < ddtrace_integrations_len; ++i) {
        ...
        dd_add_integration_to_map(name, name_len, &ddtrace_integrations[i]);
    }
...
ddtrace_integration* ddtrace_get_integration_from_string(zend_string *integration) {
    return zend_hash_find_ptr(&dd_string_to_integration_name_map, integration);
}

static void dd_add_integration_to_map(char* name, size_t name_len, ddtrace_integration* integration) {
    zend_hash_str_add_ptr(&dd_string_to_integration_name_map, name, name_len, integration);
