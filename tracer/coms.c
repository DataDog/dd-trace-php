...
/* guard our logging in the whole file */
#define datadog_signal_safe_logf(fmt, ...) (get_global_DD_TRACE_DEBUG_CURL_OUTPUT() ? datadog_signal_safe_logf(fmt, __VA_ARGS__) : 0)
 ...
 */
static ddtrace_coms_stack_t *dd_new_stack(size_t min_size) {
...
static void dd_coms_free_stack(ddtrace_coms_stack_t *stack) {
    free(stack->data);
    free(stack);
}

static void dd_recycle_stack(ddtrace_coms_stack_t *stack) {
...
static void (*dd_ptr_at_exit_callback)(void) = 0;

static void dd_at_exit_callback() { ddtrace_coms_flush_shutdown_writer_synchronous(); }

static void dd_at_exit_hook() {
    if (dd_ptr_at_exit_callback) {
        dd_ptr_at_exit_callback();
    }
}
...
bool ddtrace_coms_minit(size_t initial_stack_size, size_t max_stack_size, size_t max_backlog_size) {
    ...
    ddtrace_coms_stack_t *stack = dd_new_stack(initial_stack_size);
...
static void dd_unsafe_store_or_discard_stack(ddtrace_coms_stack_t *stack) {
    ...
    dd_coms_free_stack(stack);
}

static void dd_unsafe_cleanup_dirty_stack_area(void) {
    ddtrace_coms_stack_t *current_stack = atomic_load(&ddtrace_coms_globals.current_stack);
    ...
    if (ddtrace_coms_globals.tmp_stack != current_stack) {
        ...
        dd_unsafe_store_or_discard_stack(stack);
    }
    ddtrace_coms_globals.tmp_stack = NULL;
}
 ...
 */
static void dd_unsafe_store_or_swap_current_stack_for_empty_stack(size_t min_size) {
    dd_unsafe_cleanup_dirty_stack_area();
...
static bool dd_coms_unsafe_rotate_stack(bool attempt_allocate_new, size_t min_size) {
    dd_unsafe_store_or_swap_current_stack_for_empty_stack(min_size);
    ...
    if (!current_stack) {
        if (attempt_allocate_new) {
            ...
            *next_stack = dd_new_stack(min_size);
...
static size_t dd_write_array_header(char *buffer, size_t buffer_size, size_t position, uint32_t array_size) {
...
static size_t dd_write_to_buffer(char *buffer, size_t buffer_size, size_t position, struct _grouped_stack_t *read) {
...
static bool dd_ensure_correct_dest_capacity(struct _grouped_stack_t *dest, size_t position, size_t write_size) {
...
static void dd_write_metadata(struct _grouped_stack_t *dest, size_t position, size_t elements_in_group,
                               size_t bytes_in_group) {
    dd_ensure_correct_dest_capacity(dest, position, sizeof(size_t) * 2);
...
static void dd_read_metadata(struct _grouped_stack_t *dest, size_t position, size_t *elements_in_group,
                              size_t *bytes_in_group) {
...
static size_t dd_coms_read_callback(char *buffer, size_t size, size_t nitems, void *userdata) {
    if (!userdata) {
        return 0;
    }
    ...
    if (read->total_groups > 0) {
        written += dd_write_array_header(buffer, buffer_size, written, read->total_groups);
        read->total_groups = 0;
    }

    // write the remainder from previous iteration
    written += dd_write_to_buffer(buffer, buffer_size, written, read);
    ...
    while (written < buffer_size) {
        ...
        dd_read_metadata(read, read->position, &num_elements, &read->bytes_to_write);
...
static struct _entry_t dd_create_entry(ddtrace_coms_stack_t *stack, size_t position) {
...
static void dd_mark_entry_as_processed(struct _entry_t *entry) {
...
static size_t dd_append_entry(struct _entry_t *entry, struct _grouped_stack_t *dest, size_t position) {
    if (dd_ensure_correct_dest_capacity(dest, position, entry->size)) {
...
static void dd_msgpack_group_stack_by_id(ddtrace_coms_stack_t *stack, struct _grouped_stack_t *dest) {
    // perform an insertion sort by group_id
    uint32_t current_group_id = 0;
    struct _entry_t first_entry = dd_create_entry(stack, 0);
    dest->total_bytes = 0;
    dest->total_groups = 0;
    ...
    while (current_src_beginning < bytes_written) {
        ...
        while (current_src_position < bytes_written) {
            struct _entry_t entry = dd_create_entry(stack, current_src_position);
            if (entry.size == 0) {
                break;
            }

            if (entry.group_id == current_group_id) {
                size_t copied = dd_append_entry(&entry, dest, group_dest_position);
                if (copied > 0) {
                    dd_mark_entry_as_processed(&entry);
                    elements_in_group++;
        ...
        dd_write_metadata(dest, group_dest_beginning_position, elements_in_group, bytes_in_group);
...
static void *dd_init_read_userdata(ddtrace_coms_stack_t *stack) {
    ...
    dd_msgpack_group_stack_by_id(stack, readstack);

    return readstack;
}

static void dd_deinit_read_userdata(void *userdata) {
...
// Removable v0.4 bolt-on: selects the writer's trace endpoint (/v1.0/traces when set, else
// /v0.4/traces). Toggled per flush by auto_flush from the agent-capability check.
...
static struct timespec dd_deadline_in_ms(uint32_t ms) {
...
static size_t dd_dummy_write_callback(char *ptr, size_t size, size_t nmemb, void *userdata) {
    UNUSED(userdata);
    size_t data_length = size * nmemb;
    datadog_signal_safe_logf("%s", ptr);
    return data_length;
}

static void dd_curl_reset_headers(struct _writer_loop_data_t *writer) {
...
static void dd_curl_set_headers(struct _writer_loop_data_t *writer, size_t trace_count) {
...
static void dd_curl_send_stack(struct _writer_loop_data_t *writer, ddtrace_coms_stack_t *stack, trace_api_metrics *metrics) {
    if (!writer->curl) {
        datadog_signal_safe_logf("[bgs] no curl session - dropping the current stack.\n", NULL);
        return;
    }

    void *read_data = dd_init_read_userdata(stack);
    ...
    for (int retry = 0; retry < retries; retry++) {
        dd_curl_set_headers(writer, kData->total_groups);
        ...
        curl_easy_setopt(writer->curl, CURLOPT_WRITEFUNCTION, dd_curl_writefunc);
        curl_easy_setopt(writer->curl, CURLOPT_WRITEDATA, &response);
        res = curl_easy_perform(writer->curl);

        if (res != CURLE_OK) {
            datadog_signal_safe_logf("[bgs] curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
            ...
            curl_easy_setopt(writer->curl, CURLOPT_READFUNCTION, dd_coms_read_callback);
            curl_easy_setopt(writer->curl, CURLOPT_WRITEFUNCTION, dd_dummy_write_callback);
...
#pragma GCC diagnostic pop
                datadog_signal_safe_logf("[bgs] uploaded %.0f bytes\n", uploaded);
            }
    ...
    dd_deinit_read_userdata(read_data);
    dd_curl_reset_headers(writer);
}

static void dd_signal_writer_started(struct _writer_loop_data_t *writer) {
    if (writer->thread) {
...
static void dd_signal_writer_finished(struct _writer_loop_data_t *writer) {
    if (writer->thread) {
...
static void dd_signal_data_processed(struct _writer_loop_data_t *writer) {
    if (writer->thread) {
...
#pragma GCC diagnostic ignored "-Wclobbered"
    pthread_cleanup_push(dd_writer_loop_cleanup, writer);
#pragma GCC diagnostic pop

    bool running = true;
    dd_signal_writer_started(writer);
    ...
    do {
        ...
        if (interval > 0) {
            struct timespec wait_deadline = dd_deadline_in_ms(interval);
            if (writer->thread) {
        ...
        if (!*stack) {
            *stack = dd_coms_attempt_acquire_stack();
        }
        ...
        while (*stack) {
            ...
            if (atomic_load(&writer->sending)) {
                dd_curl_send_stack(writer, *stack, &metrics);
            }
        ...
        dd_signal_data_processed(writer);
    } while (running);

    dd_curl_reset_headers(writer);

    dd_coms_stack_shutdown();
...
static void dd_writer_set_shutdown_state(struct _writer_loop_data_t *writer) {
...
static void dd_writer_set_operational_state(struct _writer_loop_data_t *writer) {
...
static struct _writer_thread_variables_t *dd_create_thread_variables() {
...
static bool dd_coms_start_writer(void) {
    struct _writer_loop_data_t *writer = dd_get_writer();
    dd_writer_set_operational_state(writer);
    struct _writer_thread_variables_t *thread = dd_create_thread_variables();
    writer->thread = thread;
    writer->set_secbit = get_global_DD_TRACE_RETAIN_THREAD_CAPABILITIES();
    atomic_store(&writer->starting_up, true);
    return pthread_create(&thread->self, NULL, &dd_writer_loop, NULL) == 0;
}
...
bool ddtrace_coms_init_and_start_writer(void) {
    ...
    datadog_ffi_try("error creating config writer", ddog_create_agent_remote_config_writer(&dd_agent_config_writer, &ddtrace_coms_agent_config_handle));
    return dd_coms_start_writer();
}

static bool dd_has_pid_changed(void) {
    struct _writer_loop_data_t *writer = dd_get_writer();
...
bool ddtrace_coms_flush_shutdown_writer_synchronous(void) {
    ...
    if (atomic_load(&writer->starting_up) || atomic_load(&writer->running)) {
        struct timespec deadline = dd_deadline_in_ms(get_global_DD_TRACE_SHUTDOWN_TIMEOUT());
    ...
    if (should_join && !dd_has_pid_changed()) {
        ...
        if (atomic_load(&writer->running) || atomic_load(&writer->starting_up)) {
            ...
            pthread_mutex_lock(&writer->thread->writer_shutdown_signal_mutex);
            if (atomic_load(&writer->running) || atomic_load(&writer->starting_up)) {
                struct timespec grace_deadline = dd_deadline_in_ms(get_global_DD_TRACE_SHUTDOWN_TIMEOUT());
...
bool ddtrace_coms_synchronous_flush(uint32_t timeout) {
    ...
    while (previous_writer_cycle == atomic_load(&writer->writer_cycle)) {
        ...
        struct timespec deadline = dd_deadline_in_ms(timeout);
...
static void *dd_test_writer_function(void *_) {
    (void)_;
...
uint32_t ddtrace_coms_test_writers(void) {
    ...
    for (int i = 0; i < threads; i++) {
        int ret = pthread_create(&thread[i], NULL, &dd_test_writer_function, NULL);
...
uint32_t ddtrace_coms_test_consumer(void) {
    if (!dd_coms_unsafe_rotate_stack(true, atomic_load(&ddtrace_coms_globals.stack_size))) {
...
uint32_t ddtrace_coms_test_msgpack_consumer(void) {
    dd_coms_unsafe_rotate_stack(true, atomic_load(&ddtrace_coms_globals.stack_size));

    ddtrace_coms_stack_t *stack = dd_coms_attempt_acquire_stack();
    if (!stack) {
        return 0;
    }
    void *userdata = dd_init_read_userdata(stack);

    char *data = calloc(100000, 1);

    size_t written = dd_coms_read_callback(data, 1, 1000, userdata);
    if (written > 0) {
    ...
    free(data);
    dd_deinit_read_userdata(userdata);
    dd_coms_free_stack(stack);
    return 1;
}
/* }}} */
