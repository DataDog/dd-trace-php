...
static void ddtrace_routing_cache_evict_oldest(void) {
    ...
    zend_hash_internal_pointer_reset_ex(&DDTRACE_G(rcache), &pos);
    if (zend_hash_get_current_key_type_ex(&DDTRACE_G(rcache), &pos) == HASH_KEY_IS_STRING) {
        zend_hash_get_current_key_ex(&DDTRACE_G(rcache), &key, &num_idx, &pos);
        zend_hash_del(&DDTRACE_G(rcache), key);
    }
}
...
PHP_FUNCTION(DDTrace_routing_cache_get) {
    ...
    zend_string *value = zend_hash_find_ptr(&DDTRACE_G(rcache), key);
    if (!value) {
        RETURN_FALSE;
    }
...
PHP_FUNCTION(DDTrace_routing_cache_set) {
    ...
    if (zend_hash_num_elements(&DDTRACE_G(rcache)) >= DDTRACE_ROUTING_CACHE_CAPACITY
        && !zend_hash_find_ptr(&DDTRACE_G(rcache), key)) {
        ddtrace_routing_cache_evict_oldest();
    }

    zend_string *persistent_value = zend_string_init(ZSTR_VAL(value), ZSTR_LEN(value), 1);
    zend_hash_str_update_ptr(&DDTRACE_G(rcache), ZSTR_VAL(key), ZSTR_LEN(key), persistent_value);
}
