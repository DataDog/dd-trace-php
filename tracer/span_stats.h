 ...
 *
 * attributes, meta and metrics are borrowed pointers (no ownership transfer). Tag lookups follow the
 * attributes > meta > metrics precedence.
...
typedef struct {
    ...
    bool service_from_meta;  /* from_meta: an attributes or meta override */
    bool name_from_meta;
    bool resource_from_meta;
    bool type_from_meta;

    bool has_exception; /* when span->property_exception holds a Throwable */
    bool ignore_error;  /* SpanData::$ignoreError, or the deprecated meta["error.ignored"] */
 ...
 * Checks the error.message/error.type tags, pre->has_exception, and pre->ignore_error.
