...
static ddtrace_distributed_tracing_result ddtrace_read_distributed_tracing_ids_datadog(ddtrace_read_header *read_header, void *data) {
    ...
    if (read_header((zai_str)ZAI_STRL("X_DATADOG_TRACE_ID"), "x-datadog-trace-id", &trace_id_str, data)) {
        ...
        result.trace_id = (datadog_trace_id) {.low = ddtrace_parse_userland_span_id(&trace_zv)};
...
void ddtrace_apply_distributed_tracing_result(ddtrace_distributed_tracing_result *result, ddtrace_root_span_data *span) {
    ...
    if (span) {
        ...
        *Z_ARR(zv) = result->propagated_tags;
        datadog_assign_variable(&span->property_propagated_tags, &zv);
        ...
        if (result->origin) {
            ...
            datadog_assign_variable(&span->property_origin, &zv);
        }
        ...
        *Z_ARR(zv) = result->tracestate_unknown_dd_keys;
        datadog_assign_variable(&span->property_tracestate_tags, &zv);
    ...
    if (result->trace_id.low || result->trace_id.high || result->priority_sampling != DDTRACE_PRIORITY_SAMPLING_UNKNOWN) {
        if (span) {
            ...
            datadog_assign_variable(&span->property_propagated_sampling_priority, &zv);
        } else {
