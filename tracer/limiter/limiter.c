...
void ddtrace_limiter_create() {
    if (zai_config_memoized_entries[DATADOG_CONFIG_DD_TRACE_SAMPLE_RATE].name_index == ZAI_CONFIG_ORIGIN_DEFAULT) {
        return;
    }
    ...
    ddog_ShmHandle *shm;
    if (!datadog_ffi_try("Failed allocating shared memory", ddog_alloc_anon_shm_handle(limit, &shm))) {
        return;
    }
    size_t _size;
    if (!datadog_ffi_try("Failed mapping shared memory", ddog_map_shm(shm, &dd_limiter_mapped_shm, (void **)&dd_limiter, &_size))) {
