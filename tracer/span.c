...
ddtrace_inferred_span_data *ddtrace_open_inferred_span(ddtrace_inferred_proxy_result *result, ddtrace_root_span_data *root) {
    ...
    if (result->domain) {
        ...
        datadog_assign_variable(&span->property_service, &zv);
    } else {
...
DATADOG_PUBLIC zend_object *ddtrace_get_root_span()
{
    if (!DDTRACE_G(active_stack)) {
        return NULL;
    }
...
void ddtrace_close_stack_userland_spans_until(ddtrace_span_data *until) {
    ...
    while ((pspan = until->stack->active) && pspan->stack == until->stack && pspan != &until->props && SPANDATA(pspan)->type != DDTRACE_AUTOROOT_SPAN) {
        ...
        zend_string *name = datadog_convert_to_str(&span->property_name);
            ...
            if (root_span && (root_span->stack == stack || root_span->type == DDTRACE_SPAN_CLOSED)) {
                ...
                if (!get_global_DD_APM_TRACING_ENABLED()) {
                    ...
                    if (!ddtrace_standalone_limiter_allow() && !root_span->asm_event_emitted && !ddtrace_trace_source_is_meta_asm_sourced(ddtrace_property_array(&stack->root_span->property_meta))) {
                        ...
                        root_span->otel_sampling_decision = DDTRACE_OTEL_SAMPLING_DECISION_NON_PROBABILITY;
#ifdef __linux__
...
void ddtrace_close_top_span_without_stack_swap(ddtrace_span_data *span) {
    if (span->std.ce == ddtrace_ce_root_span_data) {
        ...
        bool resource_renaming_at_default = zai_config_memoized_entries[DATADOG_CONFIG_DD_TRACE_RESOURCE_RENAMING_ENABLED].name_index == ZAI_CONFIG_ORIGIN_DEFAULT;
...
zend_string *datadog_trace_id_as_hex_string(datadog_trace_id id) {
...
void ddtrace_populate_span_data(ddtrace_span_data *span, zend_string **service, zend_string **env, zend_string **version) {
    ...
    if (Z_TYPE_P(prop_service) == IS_STRING && Z_STRLEN_P(prop_service) > 0) {
        *service = zend_string_copy(Z_STR_P(prop_service));
    } else {
    ...
    if (Z_TYPE_P(prop_env) == IS_STRING && Z_STRLEN_P(prop_env) > 0) {
        *env = zend_string_copy(Z_STR_P(prop_env));
    } else {
        *env = NULL;
    }
    ...
    if (prop_version && Z_TYPE_P(prop_version) == IS_STRING && Z_STRLEN_P(prop_version) > 0) {
        *version = zend_string_copy(Z_STR_P(prop_version));
    } else {
        *version = NULL;
    }
}
...
void ddtrace_sidecar_submit_root_span_data(void) {
    if (DDTRACE_G(active_stack)) {
        ...
        if (root) {
            ddtrace_sidecar_submit_span_data_direct_defaults(&DATADOG_G(sidecar), &root->span);
        }
    }
}
