...
static void dd_exception_to_error_msg(zend_object *exception, dd_span_sink *span, enum dd_exception exception_state) {
    ...
    dd_sink_meta_str_cs(span, "error.message", (ddog_CharSlice){.ptr = error_text, .len = len});
...
static void dd_exception_to_error_type(zend_object *exception, dd_span_sink *span) {
    if (instanceof_function(exception->ce, ddtrace_ce_fatal_error)) {
        ...
        dd_sink_meta_str_str(span, "error.type", error_type_string);
    } else {
        dd_sink_meta_str_zstr(span, "error.type", exception->ce->name);
    }
}

static void dd_exception_trace_to_error_stack(zend_string *trace, dd_span_sink *span) {
    dd_sink_meta_str_zstr(span, "error.stack", trace);
...
static void ddtrace_capture_string_value(zend_string *str, struct ddog_CaptureValue *value, const ddog_CaptureConfiguration *config) {
    ...
    if (!value->not_captured_reason.len) {
        ...
        ddtrace_increase_capture_size(value->value.len);
    }
}
...
static void ddtrace_capture_long_value(zend_long num, struct ddog_CaptureValue *value) {
    ...
    if (!value->not_captured_reason.len) {
        ...
        ddtrace_increase_capture_size(len);
    }
}
...
void ddtrace_create_capture_value(zval *zv, struct ddog_CaptureValue *value, const ddog_CaptureConfiguration *config, int remaining_nesting) {
    if (UNEXPECTED(DDTRACE_G(debugger_capture_timed_out))) {
        value->not_captured_reason = dd_capture_aborted_reason();
        return;
    }
    ZVAL_DEREF(zv);
    switch (Z_TYPE_P(zv)) {
        ...
        case IS_OBJECT: {
            ...
            ZEND_HASH_REVERSE_FOREACH_STR_KEY_VAL(ht, key, val) {
                ...
                ddtrace_increase_capture_size(fieldname.len);
                ZVAL_DEINDIRECT(val);
                ddtrace_create_capture_value(val, &value_capture, config, remaining_nesting - 1);
    ...
    ddtrace_increase_capture_size(DD_CAPTURE_VALUE_OVERHEAD + value->type.len);
}

#define uuid_len 36
#define hash_len 16

static ddog_DebuggerCapture *dd_create_frame_and_collect_locals(char *exception_id, char *exception_hash, int frame_num, ddog_CharSlice class_slice, ddog_CharSlice func_slice, zval *locals, zend_string *service_name, const ddog_CaptureConfiguration *capture_config, uint64_t time, dd_span_sink *span) {
...
static bool ddtrace_exception_debugging_is_active(void) {
    return DATADOG_G(sidecar) && datadog_sidecar_instance_id && get_DD_EXCEPTION_REPLAY_ENABLED();
}

static void ddtrace_collect_exception_debug_data(zend_object *exception, zend_object *throwable, zend_string *service_name, uint64_t time, dd_span_sink *span) {
    ...
    dd_sink_meta_str_str(span, "error.debug_info_captured", "true");
    dd_sink_meta_str_cs(span, "_dd.debug.error.exception_hash", (ddog_CharSlice){.ptr = exception_hash, .len = hash_len});

    if (!ddog_exception_hash_limiter_inc(DATADOG_G(sidecar), (uint64_t)exception_long_hash, get_DD_EXCEPTION_REPLAY_CAPTURE_INTERVAL_SECONDS())) {
    ...
    ZEND_HASH_FOREACH_NUM_KEY_VAL(Z_ARR_P(trace), frame_num, frame) {
        ...
        // Every frame is a snapshot of its own; stop adding them once the capture was aborted.
        if (UNEXPECTED(DDTRACE_G(debugger_capture_timed_out))) {
            break;
        }
        ...
        ddog_CharSlice ex_msg_slice = ddtrace_capture_bound_charslice(dd_zend_string_to_CharSlice(ex_msg), capture_config.max_length);
        ddtrace_increase_capture_size(ex_msg_slice.len + ZSTR_LEN(throwable->ce->name)); // repeated on every frame
    ...
    if (get_DD_EXCEPTION_REPLAY_CAPTURE_MAX_FRAMES() < 0 || get_DD_EXCEPTION_REPLAY_CAPTURE_MAX_FRAMES() > frame_num + 1) {
        if (locals && Z_TYPE_P(locals) == IS_ARRAY) {
            ...
            ddog_CharSlice last_ex_msg_slice = ddtrace_capture_bound_charslice(dd_zend_string_to_CharSlice(last_ex_msg), capture_config.max_length);
            ddtrace_increase_capture_size(last_ex_msg_slice.len + ZSTR_LEN(throwable->ce->name));
...
void ddtrace_exception_to_meta(zend_object *exception, zend_string *service_name, uint64_t time, dd_span_sink *span, enum dd_exception exception_state) {
