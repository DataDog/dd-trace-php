...
class string_table {
    ddprof_arena **arena;  // does not own the arena
    ...
    explicit string_table(ddprof_arena **arena) noexcept;
...
inline string_table &string_table::operator=(string_table &&) noexcept =
    default;

inline string_table::string_table(ddprof_arena **a) noexcept :
    arena{a}, map{}, vector{} {
...
inline interned_string &string_table::operator[](std::size_t offset) noexcept {
    assert(offset < size());
    char *addr =
        const_cast<char *>(vector[offset] - offsetof(interned_string, data));
    return *reinterpret_cast<interned_string *>(addr);
}

inline const interned_string &string_table::operator[](
