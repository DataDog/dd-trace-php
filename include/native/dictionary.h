...
ssize_t dictionary_add(Dictionary*, unsigned char*, size_t, void*, size_t);  // An ADD operation only succeeds if the key is unpopulated (or deleted)
ssize_t dictionary_add_cstr(Dictionary*, char*, void*, size_t);
ssize_t dictionary_put(Dictionary*, unsigned char*, size_t, void*, size_t);  // A PUT operation succeeds whether or not the key is already populated
