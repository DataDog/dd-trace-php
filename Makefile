...
RUST_FILES = $(BUILD_DIR)/Cargo.toml $(shell find components-rs -name '*.c' -o -name '*.rs' -o -name 'Cargo.toml' | awk '{ printf "$(BUILD_DIR)/%s\n", $$1 }' ) $(shell find libdatadog/{alloc,bin_tests,build-common,crashtracker,crashtracker-ffi,data-pipeline,data-pipeline-ffi,ddcommon,ddcommon-ffi,ddsketch,ddtelemetry,ddtelemetry-ffi,dynamic-configuration,ipc,profiling,profiling-ffi,profiling-replayer,remote-config,serverless,sidecar,sidecar-ffi,spawn_worker,symbolizer-ffi,tests,tools,trace-*,Cargo.toml} -type f \( -path "*/src*" -o -path "*/examples*" -o -path "*Cargo.toml" -o -path "*/build.rs" -o -path "*/tests/dataservice.rs" -o -path "*/tests/service_functional.rs" -o -path "*/benches/*.rs" \) -not -path "*/ipc/build.rs" -not -path "*/sidecar-ffi/build.rs")
