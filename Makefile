...
$(BUILD_DIR)/configure: $(BUILD_DIR)/config.m4
	$(Q) (cd $(BUILD_DIR); phpize && sed -i 's/\/FAILED/\/\\bFAILED/' run-tests.php) # Fix PHP 5.4 exit code bug when running selected tests (FAILED vs XFAILED)
...
test_extension_ci: $(SO_FILE) $(TEST_FILES) $(TEST_STUB_FILES)
	...
	cd $(BUILD_DIR); $(RUN_TESTS_CMD) -d extension=$(SO_FILE) -m -s $$TEST_PHP_OUTPUT $(BUILD_DIR)/$(TESTS) && ! grep -e 'LEAKED TEST SUMMARY' $$TEST_PHP_OUTPUT; \
	)
